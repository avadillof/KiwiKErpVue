...
const stats = ref({
    totalRecords: 0, // Mantenlo como número aquí
    dailyAverage: '0.0'
});
...
// Define una estructura segura
const chartData = ref({
    labels: [],
    datasets: [{
        label: 'Conexiones',
        data: [], // Aquí TypeScript verá que es un array (number[])
        borderColor: '#3B82F6', // Color por defecto
        tension: 0.4,
        fill: false
    }]
});
...
const open = async (userId: number, userName: string) => {
    ...
    // 1. Llamada a estadísticas (Protegida)
    try {
        const resStats = await fetch(`https://localhost:8083/api/historicuser/${userId}/stats`);
        if (!resStats.ok) throw new Error(`Stats HTTP error! status: ${resStats.status}`);
        
        const data = await resStats.json();

        stats.value = {
            totalRecords: Number(data.totalRecords),
            dailyAverage: Number(data.dailyAverage || 0).toFixed(1).replace('.', ',')
        };
    } catch (e) { 
        console.error("Error al cargar stats:", e); 
    }

    // 2. Llamada a datos de la gráfica (Protegida)
    try {
        const resChart = await fetch(`https://localhost:8083/api/historicuser/${userId}/chart-data`);
        if (!resChart.ok) throw new Error(`Chart HTTP error! status: ${resChart.status}`);
        
        const data = await resChart.json();

        // VALIDACIÓN: Solo asignamos si hay datos reales
        ...
        console.error("Error al cargar chart:", e);
        // Opcional: limpiar chartData para evitar renderizados rotos
...
const formattedTotalRecords = computed(() => {
    const val = stats.value.totalRecords;
    // Convertimos a string, eliminamos cualquier formato previo
    const str = val.toString().replace(/\D/g, '');
    // Aplicamos una expresión regular para añadir el punto cada 3 dígitos
