...
const updateChart = (data: ConnectionLog[]) => {
    ...
    data.forEach((item: ConnectionLog) => {
        // Tu fecha: "jun. 06, 2026..."
        // Vamos a extraer año, mes y día para crear una clave única
        try {
            // Ejemplo: "jun. 06, 2026"
            const parts = item.date.split(' '); 
            const mesStr = parts[0].replace('.', ''); // "jun"
            const diaStr = parts[1].replace(',', ''); // "06"
            const anioStr = parts[2];                 // "2026"

            // Convertimos mes a número (0-11)
            const meses: Record<string, string> = { 
                'ene': '01', 'feb': '02', 'mar': '03', 'abr': '04', 'may': '05', 'jun': '06', 
                'jul': '07', 'ago': '08', 'sep': '09', 'oct': '10', 'nov': '11', 'dic': '12' 
            };
            const mesNum = meses[mesStr.toLowerCase()] || '01';

            // Clave única cronológica: "2026-06-06"
            const dateKey = `${anioStr}-${mesNum}-${diaStr.padStart(2, '0')}`;
