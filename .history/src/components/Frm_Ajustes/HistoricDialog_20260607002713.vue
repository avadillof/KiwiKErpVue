...
const handleDataLoaded = (data: ConnectionLog[]) => {
    ...
    // Siempre actualizamos el gráfico basándonos en el PRIMER registro 
    // de la lista actual (que es el que se ve arriba de la tabla)
    if (allLoadedData.value.length > 0) {
        const fechaReferencia = new Date(parseToDateKey(allLoadedData.value[0].date));
        console.log("Actualizando gráfico con fecha:", fechaReferencia);
        updateChart(allLoadedData.value, fechaReferencia);
    }
};

// Esta función captura cuando el usuario hace scroll en la tabla
const onTableScroll = (event: any) => {
    // PrimeVue DataTable suele pasar el objeto scroll en el evento
    // Pero si no, podemos calcular el índice basado en la posición
    const scrollTop = event.target.scrollTop;
    const rowHeight = 50; // Ajusta según la altura aproximada de tus filas
    const index = Math.floor(scrollTop / rowHeight);

    if (allLoadedData.value[index]) {
        console.log("Detectado scroll en índice:", index);
