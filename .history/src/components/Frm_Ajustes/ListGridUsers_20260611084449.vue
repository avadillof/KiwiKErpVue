<template>
  <div class="card" style="height: 500px; display: flex; flex-direction: column;">
    ...
    <GenericDataTable style="min-height: 400px;" ref="tableRef" dataKey="userKyId" :endpoint="apiUrl"
      :showPaginator="true" :filterable="true" selectionMode="single" v-model:selection="miSeleccion"
...
const miSeleccion = ref<User | null>(null);

const menuItems = ref([
  { label: 'Editar', icon: 'pi pi-pencil', command: () => emit('edit', miSeleccion.value) },
  {
    label: 'Historial Conexiones',
    icon: 'pi pi-history',
    command: () => {
      
      if (!miSeleccion.value) {
        console.error("No hay usuario seleccionado");
        return;
      }

      historicDialogRef.value.open(
       miSeleccion.value.pkid,
       miSeleccion.value.name
      );
    }
  },
  { separator: true },
  { label: 'Borrar', icon: 'pi pi-trash', style: 'color: var(--red-500)', command: () => console.log('Borrar', miSeleccion.value) }
]);
