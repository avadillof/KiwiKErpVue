<template>
  <div class="card" style="height: 500px; display: flex; flex-direction: column;">
    ...
    <DataTable v-model:selection="selectedUser" :value="usersStore.allUsers" :loading="usersStore.loading"
      ...
      <template #body="slotProps">
        <Badge 
          :value="slotProps.data.userGroup.descriptionEs" 
          :severity="getRoleSeverity(slotProps.data.userGroup.descriptionEs)"
        />
      </template>
    </Column>
      ...
      <Column header="Acciones" style="width: 10%; text-align: center;">
        <template #body="slotProps">
          <Button icon="pi pi-pencil" severity="info" text rounded @click.stop="emitEdit(slotProps.data)" />
        </template>
      </Column>
    </DataTable>
  </div>
...
function getRoleSeverity(roleName: string) {
        // Convertimos a minúsculas para comparar sin errores de acentos o mayúsculas
        const name = roleName?.toLowerCase() || '';

        if (name.includes('administrador') || name.includes('administrator')) {
            return 'danger';  // Rojo para administradores
        }
        if (name.includes('tpv')) {
            return 'warn';    // Naranja para TPVs
        }
        return 'info';      // Azul para usuarios estándar
    }
</script>