<template>
    <Dialog v-model:visible="visible" modal :header="'Análisis de Actividad para ' + selectedUserName"
        :style="{ width: '1000px', height: '90vh' }" class="kiwik-dialog" :dismissableMask="true">
                    ...
                    <div class="stat-content">
                        <p class="stat-label">Total Registros</p>
                           <strong class="stat-value">
                                {{ formatThousands(stats.totalRecords) }}
                        </strong>
                    </div>
                </div>
                <div class="stat-card">
...
.stat-label {
    margin: 0;
    font-size: 0.9rem; /* Un poco más grande que el default */
    color: #6b7280;    /* Gris medio para no competir con el valor */
    font-weight: 500;
}

.stat-value {
    font-size: 1.6rem; /* Aquí está el tamaño grande que buscas */
    color: #111827;    /* Gris muy oscuro/casi negro para máxima legibilidad */
    line-height: 1.2;
    margin-top: 0.25rem;
}
...
const formatThousands = (value: any) => {
  // 1. Limpiamos: Quitamos espacios y convertimos a número
  // Si viniera algo como "1.574", el parseFloat ayuda a procesarlo
  const num = parseFloat(String(value).replace(/\s/g, '').replace(/\./g, '').replace(',', '.'));
  
  // 2. Si el resultado no es un número, devolvemos 0
