...
// Conversión segura de string a objeto Date para evitar errores de TS
...
const updateChart = (data: any[]) => {
    ...
    // Usar el rango de los datos cargados para estabilizar la gráfica
...
const calculateDailyAverage = () => {
    // Si aún no hemos cargado datos, o el total es 0
    if (totalRecordsFromAPI.value === 0) return '0.0';

    // Para que esto sea 100% preciso, necesitamos saber cuántos días únicos hay
    // en la BD. Si no puedes saberlo, la aproximación más estable es:
    const uniqueDays = new Set(allLoadedData.value.map(item => parseToDateKey(item.date)));
    
    // Aquí el secreto: usar el total de registros de la BD dividido por los días cargados
    // Esto hará que la media sea mucho más estable aunque el scroll no haya terminado
