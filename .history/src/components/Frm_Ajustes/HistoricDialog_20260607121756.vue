...
const updateChart = (data: any[]) => {
    ...
    data.forEach(item => {
        if (!item.date) return;
        const key = parseToDateKey(item.date); // Esto ya lo tenías, asegúrate que funcione
        counts[key] = (counts[key] || 0) + 1;
    });

    // Ordenamos las fechas para que la línea sea coherente
...
const handleDataLoaded = (data: any[], total: number) => {
    ...
    const datosFiltrados = data.filter(item => item !== null);

    if (datosFiltrados.length > 0) {
        updateChart(datosFiltrados);
    }
};
...
const open = async (userId: number, userName: string) => {
    ...
    try {
        ...
        // Accedemos a data.content, que es donde está el array real
        if (data && Array.isArray(data.content)) {
            ...
            chartData.value = {
                ...
                datasets: [{
                    ...chartData.value.datasets[0],
                    data: items.map((d: any) => d.count) // Asegúrate de que 'count' sea el nombre correcto del campo
                }]
            };
        } else {
            console.warn("La estructura esperada no contiene un array en 'content':", data);
        }
    } catch (e) {
