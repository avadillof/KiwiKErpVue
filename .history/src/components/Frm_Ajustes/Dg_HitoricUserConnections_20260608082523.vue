...
const lastDate = computed(() => {
    // Verificamos si existe el array y si tiene al menos un elemento
    const firstItem = data.value?.[0];
    
    // Verificamos si el primer elemento tiene la propiedad 'date'
    return firstItem?.date         ? parseToDateKey(firstItem.date) 
        : '-';
});
