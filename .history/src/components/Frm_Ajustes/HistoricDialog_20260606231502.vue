...
const updateChart = (data: ConnectionLog[]) => {
    // 1. Agrupamos los conteos por fecha completa (YYYY-MM-DD)
    const countsMap: Record<string, number> = {};
    
    data.forEach((item: ConnectionLog) => {
        // Obtenemos una fecha válida para extraer el formato ISO (YYYY-MM-DD)
        // Como tu formato es "jun. 06, 2026", lo limpiamos:
        const cleanDate = item.date.replace('p. m.', 'PM').replace('a. m.', 'AM');
        const dateObj = new Date(cleanDate);
        
        if (!isNaN(dateObj.getTime())) {
            const dateKey = dateObj.toISOString().split('T')[0]; // "2026-06-06"
            countsMap[dateKey] = (countsMap[dateKey] || 0) + 1;
        }
    });

    // 2. Ordenamos las fechas y separamos en etiquetas y valores
