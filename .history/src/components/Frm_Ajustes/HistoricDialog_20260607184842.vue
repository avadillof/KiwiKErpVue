...
<style scoped>
/* 1. Contenedor principal del diálogo */
.reset-container {
    display: flex;
    flex-direction: column;
    height: 100%; /* Ocupa el 100% del Dialog */
    min-height: 0;
    overflow: hidden;
}

/* 2. Grid que distribuye Gráfica y Tabla */
.main-grid {
    display: grid;
    grid-template-columns: 1fr 0.45fr;
    gap: 1.5rem;
    flex: 1; /* Esto le dice: "ocupa todo el espacio disponible que sobre" */
    min-height: 0; /* CRÍTICO: Permite que el grid se encoja si el espacio es menor */
    margin-bottom: 1rem;
}
...
.panel {
    display: flex;
    flex-direction: column;
    overflow: hidden; /* Corta cualquier contenido interno que se desborde */
    background: var(--surface-card);
    border: 1px solid rgba(0, 0, 0, 0.08);
    border-radius: 16px;
    padding: 1.25rem;
    min-height: 0; /* Permite que el panel se adapte al grid */
}

/* 4. Contenedor del DataTable para que ocupe el panel */
...
.date-badge { background: var(--surface-hover); padding: 0.1rem 0.35rem; border-radius: 4px; font-size: 0.75rem; color: var(--text-color-secondary); }
</style>