<template>
  ...
  <Dialog v-model:visible="visible" modal header="Historial de Accesos" :style="{ width: '350px' }">
      ...
      <Timeline :value="history">
        <template #content="slotProps">
          <small class="text-color-secondary">
            {{ HelperDates.formatDateFromLocale(slotProps.item.date) }}
          </small>
        </template>
      </Timeline>
    </div>
...
const menuItems = ref([
  ...
  { label: 'Historial', icon: 'pi pi-history', command: () => console.log('Historial', selectedUser.value) },
  { separator: true },
