...
const open = async (userId: number, userName: string) => {
    ...
    try {
        ...
        // Aplicamos el redondeo aquí para asegurar una visualización limpia

        const formatter = new Intl.NumberFormat('es-ES');

        stats.value = {
            totalRecords: formatter.format(data.totalRecords),
            dailyAverage: parseFloat(data.dailyAverage).toFixed(1)
        };
