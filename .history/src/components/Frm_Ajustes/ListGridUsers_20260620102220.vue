<template>
  ...
  <template>
  <ConfirmDialog></ConfirmDialog> <Menu ref="menuOptionRegistro" :model="menuItems" :popup="true" />
</template>
...
const menuItems = ref([
  { label: 'Editar', icon: 'pi pi-pencil', command: () => userFormRef.value.open(userSelected.value?.pkid) },
  {
...
const menuItemsTable = ref<MenuItem[]>([
  {
    ...
    items: [
      ...
      { separator: true },
      {
        ...
        command: () => deleteUser() // <--- LLAMADA A LA FUNCIÓN
      }
    ]
  }
]);
...
const deleteUser = () => {
  ...
  confirm.require({
    ...
    accept: async () => {
      try {
        await axios.post(`${import.meta.env.VITE_API_URL}/WebDeleteUser`, {
          pkid: userSelected.value!.pkid
        });

        toast.add({ severity: 'success', summary: 'Borrado', detail: 'Usuario eliminado correctamente', life: 3000 });
        refreshTable(); // Recargamos la tabla automáticamente
      } catch (error) {
