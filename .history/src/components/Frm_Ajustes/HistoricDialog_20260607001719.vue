...
const handleDataLoaded = (data: ConnectionLog[]) => {
    const isFirstLoad = allLoadedData.value.length === 0;
    allLoadedData.value = [...allLoadedData.value, ...data];
    if (isFirstLoad && allLoadedData.value.length > 0) {
...
const onTablePageChange = (event: any) => {
    ...
    if (allLoadedData.value[index]) {
        updateChart(allLoadedData.value, new Date(parseToDateKey(allLoadedData.value[index].date)));
    }
};
