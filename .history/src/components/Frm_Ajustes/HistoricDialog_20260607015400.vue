    ...
    <div class="reset-container">
      <div class="stats-row">
        <div class="stat-card">
          ...
          <div>
            <p>Total Registros</p>
            <strong>{{ totalRecordsFromAPI }}</strong>
          </div>
        </div>
        <div class="stat-card">
...
const parseToDateObj = (dateStr: string): Date => {
    try {
        ...
        return new Date(parseInt(parts[2]), mesMap[parts[0].replace('.', '').toLowerCase()] || 0, parseInt(parts[1].replace(',', '')));
...
const onTableLazyLoad = (_event: any) => updateChart(allLoadedData.value);

const open = (userId: number, userName: string) => {
    selectedUserName.value = userName;
    allLoadedData.value = [];
    totalRecordsFromAPI.value = 0;
    apiUrl.value = `https://localhost:8083/api/historicuser/${userId}`;
    visible.value = true;
};

const calculateDailyAverage = () => {
    if (totalRecordsFromAPI.value === 0) return '0.0';
    const uniqueDays = new Set(allLoadedData.value.map(item => parseToDateKey(item.date)));
    // Usamos el total real de registros dividido por los días únicos encontrados
    return (totalRecordsFromAPI.value / (uniqueDays.size || 1)).toFixed(1);
};
...
<style scoped>
/* Estilos mantenidos como en la estructura anterior */
...
.main-grid { display: grid; grid-template-columns: 1fr 0.45fr; gap: 1.5rem; height: 350px; }
.panel { display: flex; flex-direction: column; overflow: hidden; background: var(--surface-card); border: 1px solid rgba(0, 0, 0, 0.08); border-radius: 16px; padding: 1.25rem; }
