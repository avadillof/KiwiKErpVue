...
const updateChart = (data: ConnectionLog[]) => {
    ...
    data.forEach((item: ConnectionLog) => {
        // En lugar de new Date(item.date), vamos a extraer el día 
        // usando el formato que ya tienes: "jun. 06, 2026..."
        
        // Opción A: Si HelperDates tiene un método para convertir esto:
        // const d = HelperDates.parseToDate(item.date);
        
        // Opción B: Si no, extraemos el día manualmente ya que el formato es consistente:
        // Ejemplo: "jun. 06, 2026" -> el día está después de la coma
        const dayMatch = item.date.match(/(\d{1,2})/); 
