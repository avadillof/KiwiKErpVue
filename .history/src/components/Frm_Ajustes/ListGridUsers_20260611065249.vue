<template>
  <div class="card" style="height: 500px; display: flex; flex-direction: column;">
    ...
    <GenericDataTable 
      ...
      :showPaginator="true"
      :filterable="true" 
      selectionMode="single" v-model:selection="miSeleccion" @row-select="handleRowSelect"
      ...
      <Column field="lastConexion" sortable style="width: 15%">
        <template #header>
          <div class="flex align-items-center justify-content-between w-full">
            <span>Última Conexión</span>
            <Button icon="pi pi-filter" text rounded size="small" @click="toggleDateFilter($event)" />
          </div>
        </template>
...
const refreshTableFilter = (filter: string) => {
  if (tableRef.value) {
    obtenerFechas();
...
watch(fechaFiltro, (nuevoValor) => {
  ...
  if (fechaFiltro.value && fechaFiltro.value[0] && fechaFiltro.value[1]) {
    const filtros = {
      startDate: fechaFiltro.value[0].toISOString(),
      endDate: fechaFiltro.value[1].toISOString()
    };
    // Suponiendo que tu componente tiene una propiedad para refrescar con filtros
    tableRef.value.refreshFilter('[' + fechaFiltro.value[0].toISOString() + '],[' + fechaFiltro.value[1].toISOString() + ']');
...
const obtenerFechas = () => {
  if (fechaFiltro.value && fechaFiltro.value[0] && fechaFiltro.value[1]) {
    const inicio = fechaFiltro.value[0];
    const fin = fechaFiltro.value[1];

    console.log("Inicio:", inicio);
    console.log("Fin:", fin);
  } else {
    console.log("El rango está vacío o incompleto");
  }
};
