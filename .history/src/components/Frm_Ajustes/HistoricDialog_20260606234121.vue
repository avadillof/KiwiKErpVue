...
const updateChart = (data: ConnectionLog[]) => {
    ...
    // 1. Agrupamos los datos existentes
    const countsMap: Record<string, number> = {};
    data.forEach(item => {
        // ... (tu lógica actual de extraer YYYY-MM-DD)
        const dateKey = obtenerFechaKey(item.date); // tu lógica de parseo
        countsMap[dateKey] = (countsMap[dateKey] || 0) + 1;
    });

    // 2. Ordenamos las fechas existentes para saber el rango
    const sortedDates = Object.keys(countsMap).sort();
    const startDate = new Date(sortedDates[0]);
    const endDate = new Date(sortedDates[sortedDates.length - 1]);

    // 3. Rellenamos los días que faltan con 0
    const finalLabels: string[] = [];
    const finalData: number[] = [];
    
    let currentDate = new Date(startDate);
    while (currentDate <= endDate) {
        const key = currentDate.toISOString().split('T')[0]; // "2026-01-16"
        finalLabels.push(key.substring(8, 10) + "/" + key.substring(5, 7)); // "16/01"
        finalData.push(countsMap[key] || 0); // Si no hubo actividad, ponemos 0
        
        currentDate.setDate(currentDate.getDate() + 1); // Sumamos un día
    }
    ...
    // Etiquetas: mostramos "06/06" (día/mes) para que sea claro
    const labels = sortedDates.map(dateKey => {
        const [anio, mes, dia] = dateKey.split('-');
        return `${dia}/${mes}`;
    });

    const counts = sortedDates.map(dateKey => countsMap[dateKey]);
