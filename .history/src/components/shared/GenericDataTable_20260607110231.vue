...
const virtualScrollerOptions = {
  lazy: true,
  itemSize: 50,
  delay: 500, // Aumenta el delay para evitar disparos instantáneos al montar
  onLazyLoad: (event: any) => {
    // BLINDAJE: Si el evento no tiene datos válidos, abortamos.
    // Esto mata la petición page=NaN antes de que salga del navegador.
    if (!event || typeof event.first !== 'number' || isNaN(event.first)) {
        console.warn("Evento de scroll ignorado por invalidez");
        return;
    }
...
onMounted(() => {
    // Disparamos la carga inicial para que el DataTable tenga algo que mostrar
    // y el v-if se vuelva true
    fetchData(0, 50);
});
