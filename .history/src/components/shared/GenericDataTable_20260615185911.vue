<template>
  <div class="table-container">
    ...
    <div class="datatable-wrapper">
      ...
      <DataTable 
         v-model:selection="selection" :selectionMode="props.selectionMode" :value="tableData"
        :loading="loading" lazy scrollable scrollHeight="flex" class="compact-table"
...
const emit = defineEmits(['update:selection', 'row-select', 'row-unselect', 'data-loaded', 'sort', 'search']);
...
async function fetchDataDateField(page: number, size: number,fechaFiltro = ref<[Date | null, Date | null] | null>) {
  loading.value = true;
  try {
    // 1. Crear params base
    const params: any = { page, size };
    ...
    // 2. Si hay filtro, lo añadimos codificado directamente
    if (props.filterable && fechaFiltro) {
      
        const [inicio, fin,aa] = fechaFiltro;
        const filtros = {
                    field: 'date',
        ...
        //tableRef.value.searchQuery=`[${filtros.field}],[${filtros.startDate}],[${filtros.endDate}]`;



      //params.query = encodeURIComponent(fechaFiltro.value);
    }
...
defineExpose({
  ...
  refreshDataField: () => {
    const currentPageIndex = Math.floor(first.value / rows.value);
    fetchDataDateField(currentPageIndex, rows.value);
  },
