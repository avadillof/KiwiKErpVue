...
async function fetchData(page: number, size: number, sortField?: string, sortOrder?: number) {
  loading.value = true;
  try {

    if (sortField) {
      params.sort = sortField;
      // PrimeVue envía 1 para ASC y -1 para DESC
      params.order = sortOrder === 1 ? 'asc' : 'desc';
    }
