...
async function fetchData(first: number, rows: number) {
  ...
  if (isFetching.value || loadedRanges.value.has(key)) return;
  ...
  try {
    ...
    // 🔥 inicialización correcta del VirtualScroll
    ...
    // 🔥 MODO CORRECTO: por posiciones (NO append)
...
const virtualScrollerOptions = computed(() => ({
  ...
  onLazyLoad: (event: any) => {
    ...
    // 🔥 FIX CRÍTICO (evita limit=0 y bugs de PrimeVue)
    if (rows <= 0) return;
    if (first < 0) return;
