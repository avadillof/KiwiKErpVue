...
async function fetchData(first: number, rows: number) {
  ...
  // 🔥 SOLO bloquea si ya estás cargando (NO por páginas)
  ...
  try {
    ...
    // 🔥 RESET SOLO cuando cambia total
    if (totalRecords.value !== totalElements) {
      totalRecords.value = totalElements;

      // ❌ NO fill(null) → rompe gráficos y sort
      tableData.value = [];

      loadedPages.value = [];
    }

    // 🔥 Seguridad total contra nulls del backend
    const safeContent = (content || []).filter(
      (item: any) => item !== null && item !== undefined
    );

    // 🔥 Insertado en posición virtual scroll
    ...
    // 🔥 marca página como cargada
    if (!loadedPages.value.includes(page)) {
      loadedPages.value.push(page);
    }

    // 🔥 EMIT LIMPIO (SIN NULLS)
    const cleanData = tableData.value.filter(
      (x) => x !== null && x !== undefined
    );

    emit('data-loaded', cleanData, totalElements);
