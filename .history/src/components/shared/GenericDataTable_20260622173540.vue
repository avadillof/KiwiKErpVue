...
async function fetchBooleanDateField(page: number, size: number, field: string, fechaFiltro: [Date | null, Date | null] | null) {
  loading.value = true;
...
defineExpose({
  ...
  refreshBooleanField: (field: string, value: boolean | null) => {
    ...
    const currentPageIndex = Math.floor(first.value / rows.value);
    fetchBooleanDateField(currentPageIndex, rows.value, field, value);
  },
