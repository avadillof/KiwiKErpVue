...
import { query } from 'node_modules/axios/index.d.cts';
...
async function fetchDataWithFilter(page: number, size: number,filter:string) {
  loading.value = true;
  try {
    // 1. Crear params base
    const params: any = { page, size };
    ...
    console.info("Parámetros enviados a Axios:", params);
    const baseUrl = import.meta.env.VITE_API_URL;
    const response = await axios.get(  baseUrl +'/'+  props.endpoint, { params });
...
defineExpose({
  ...
  refreshFilter: (filter: string) => {   
    ...
    fetchDataWithFilter(currentPageIndex, rows.value,filter);
    query.
