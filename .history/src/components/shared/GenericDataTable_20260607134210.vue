<template>
  <DataTable v-model:selection="selection" selectionMode="single" :metaKeySelection="false" :value="tableData"
    ...
    scrollHeight="flex" dataKey="id" class="compact-table" @row-select="onRowSelect"
    sortMode="single"
    itemSize: 36
    ...
    // Si tu página es de 50, el buffer ayuda a pedir la siguiente 
    // antes de llegar al final real del scroll.
...
const onSort = (event: any) => {
    // event.sortField es la columna, event.sortOrder es 1 (ASC) o -1 (DESC)
    console.log("Ordenando por:", event.sortField, event.sortOrder);
    emit('sort', event);
    
    // IMPORTANTE: Al ordenar, debemos resetear la paginación y recargar
    loadedPages.value = [];
    fetchData(0, 50); 
};
