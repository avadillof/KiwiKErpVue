<template>
  <div class="table-container">
    ...
    <div class="datatable-wrapper">
      ...
      <DataTable stripedRows v-model:selection="selection" :selectionMode="props.selectionMode" :value="tableData"
...
const sortFieldsMap = ref<Record<string, string>>({});
...
async function fetchData(page: number, size: number) {
  ...
  console.log('PARAMS TABLA', params);
...
const onSort = (event: any) => {
  ...
  sortField.value =
    sortFieldsMap.value[event.sortField] ??
    event.sortField;
...
defineExpose({
  ...
  refresh: async () => {
    ...
    return await fetchData(currentPageIndex, rows.value);
  },



  refreshWithQuery:async () => (
    query: string,
    extraParams: any = {}
  ) => {
    ...
    return await fetchDataWithQuery(
      currentPageIndex,
      rows.value,
      query,
      extraParams
...
onMounted(() => {
    ...
    defaultSlot.forEach((vnode: any) => {
        ...
        if (props?.field) {
            ...
            sortFieldsMap.value[props.field] =
