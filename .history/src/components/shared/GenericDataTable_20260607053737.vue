<template>
  <DataTable :value="data" :totalRecords="totalRecords" lazy :loading="loading"
    :virtualScrollerOptions="virtualScrollerOptions" scrollable scrollHeight="flex" v-bind="$attrs" dataKey="id">
    <slot></slot>
...
async function fetchData(first: number, rows: number) {
  ...
  try {
    ...
    // AÑADE ESTO:
    console.log("Datos en data.value tras fetch:", data.value.length);
    console.log("Ejemplo de dato en índice 0:", data.value[0]);

    // 4. Actualizamos el array. Al usar splice, el VirtualScroller 
    // entiende perfectamente que esas posiciones ya tienen datos y no placeholders.
    data.value = _newData;
