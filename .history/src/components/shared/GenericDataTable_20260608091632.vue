...
async function fetchData(page: number, size: number) {
  ...
  try {
    ...
    if (sortField) {
      // Spring espera: sort=campo,direccion
...
const onSort = (event: any) => {
  // event.sortField es el nombre del campo (ej: 'date')
  // event.sortOrder es 1 (ASC) o -1 (DESC)
  sortField.value = event.sortField;
  sortOrder.value = event.sortOrder;
  
  // Reseteamos a la página 0 porque al ordenar, la lista cambia
  first.value = 0; 
  
  // Recargamos los datos con los nuevos parámetros de orden
