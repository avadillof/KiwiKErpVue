...
const virtualScrollerOptions = {
  lazy: true, 
  itemSize: 50, // ¡IMPORTANTE! Esto debe coincidir exactamente con la altura CSS de tu fila
  delay: 0,     // A 0 para que no haya latencia en el cálculo
  showLoader: true,
  onLazyLoad: (event: any) => { 
    // Forzamos el alineamiento del índice al múltiplo de 50
    const alignedFirst = Math.floor(event.first / 50) * 50;
    fetchData(alignedFirst, event.rows); 
  },
};
