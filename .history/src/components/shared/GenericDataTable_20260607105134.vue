...
const virtualScrollerOptions = {
  ...
  onLazyLoad: (event: any) => {
    // FILTRO DE EMERGENCIA: Si no es un número, ignoramos el evento
    const first = parseInt(event.first, 10);
    const rows = parseInt(event.rows, 10);
    
    if (isNaN(first) || isNaN(rows)) {
        console.error("Bloqueado evento de scroll inválido:", event);
        return;
    }
