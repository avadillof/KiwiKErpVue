...
const booleanFiltros = ref<boolean | null>(null);
...
async function fetchBooleanField(page: number, size: number, field: string, value: boolean | null) {
  loading.value = true;

  try {


    booleanFiltros.value = value;
    searchQuery.value = '';
    ...
    if (value !== null) {
      ...
      params.query = ""; // O simplemente no asignar
    }
...
// Nueva función interna en GenericDataTable
async function fetchDataWithQuery(page: number, size: number, query: string) {
    loading.value = true;
    try {
        const params: any = { page, size, query }; // Aquí inyectamos todo el bloque de filtros
        const baseUrl = import.meta.env.VITE_API_URL;
...
const onSearch = () => {
  ...
  debounceTimer = setTimeout(() => {
    first.value = 0;
    // Emitimos el valor al padre para que él sepa que debe limpiar sus filtros
...
defineExpose({
  ...
  refreshBooleanField: (field: string, value: boolean | null) => {
    const currentPageIndex = Math.floor(first.value / rows.value);
    fetchBooleanField(currentPageIndex, rows.value, field, value);
  },


  refreshWithQuery: (query: string) => {
    const currentPageIndex = Math.floor(first.value / rows.value);

    // Nueva función que carga datos usando la query compuesta
    fetchDataWithQuery(currentPageIndex, rows.value, query);
  },
