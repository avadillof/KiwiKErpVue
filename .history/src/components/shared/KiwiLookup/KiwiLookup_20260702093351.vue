        ...
        <Dialog v-model:visible="visible" modal :style="{ width: width, height: '85vh' }" class="kiwik-dialog">
            ...
            <div class="kiwi-lookup-body">
                ...
                <div class="lookup-search">
                    <InputGroup>
                        <InputText v-model="search" :placeholder="placeholder" class="w-full" />
                    </InputGroup>
                </div>

                <!-- TABLE -->
                <div class="lookup-table">
                    <DataTable :value="filteredItems" paginator :rows="rows" selectionMode="single"
                        v-model:selection="selectedRow" :dataKey="dataKey" @row-dblclick="selectRow" scrollable
                        scrollHeight="flex">
...
/* =========================
   SYNC SINGLE SOURCE OF TRUTH (CORREGIDO)
========================= */
// Función unificada para evitar duplicidad y carreras de reactividad
const syncSelection = () => {
    ...
    // Solo buscamos si el ítem actual no coincide con el valor del modelo
    const currentId = selectedItem.value?.[props.dataKey];
    if (currentId !== props.modelValue) {
        const found = props.items.find(i => i?.[props.dataKey] === props.modelValue);
        selectedItem.value = found || null;
    }
};

// Observar ambos cambios con una sola lógica
