  ...
  <div class="table-container">
    ...
    <div class="datatable-wrapper">
      ...
      <DataTable v-model:selection="selection" :selectionMode="props.selectionMode" :value="tableData"
        :loading="loading" lazy scrollable scrollHeight="flex"  @row-select="onRowSelect"
        @sort="onSort"  class="compact-table custom-header-table" >
        <slot />
        <template #empty>
    ...
    <div class="footer-container">
      <div class="info-bar">
        ...
        <div v-if="booleanHashFiltros == true" class="info-right-group">
...
async function fetchData(page: number, size: number) {
  ...
  if (booleanHashFiltrosFields.value == false) {
...
async function fetchDataWithQuery(page: number, size: number, query: string,extraParams: any = {}) {
  loading.value = true;
  try {

    const params: any = { page, size, query,...extraParams }; // Aquí inyectamos todo el bloque de filtros

    const baseUrl = import.meta.env.VITE_API_URL;

    if (query.length == 0) {
      booleanHashFiltros.value = false;
      booleanHashFiltrosFields.value = false;
    } else {
      booleanHashFiltros.value = true;
      searchQuery.value = '';
      booleanHashFiltrosFields.value = true;
    }
