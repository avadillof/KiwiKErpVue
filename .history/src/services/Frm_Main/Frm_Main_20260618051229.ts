...
export default defineComponent({
    ...
    setup() {
        const userFormRef = ref<any>(null); // Definida aquí
        ...
        function verPerfilUsuario(): void {
            // Accedemos a la propiedad que inyectamos desde el layout
            const refForm = (this as any).userFormRef;

            console.info('Ref actual:', refForm?.value);

            if (refForm?.value && authStore.user) {
                refForm.value.open(authStore.user);
            } else {
                console.error("La referencia sigue siendo null");
            }
        }
