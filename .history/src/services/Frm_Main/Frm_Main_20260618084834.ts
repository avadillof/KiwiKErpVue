...
export default defineComponent({
    ...
    setup() {
        ...
        // En Frm_Main.ts
        function verPerfilUsuario(openForm: Function): void {
            // Verificamos que el usuario exista y tenga un pkid válido
            if (authStore.user && authStore.user.pkid) {
                // Pasamos SOLO el número (pkid), no el objeto entero
                openForm(authStore.user.pkid);
            } else {
                console.error("No hay un ID de usuario válido para abrir el formulario.");
            }
        }
    ...
    console.info('Ref actual:', userFormRef.value); 
    
    // VERIFICACIÓN: authStore.user debe tener un pkid
    if (userFormRef.value && authStore.user && authStore.user.pkid) {
        // ENVIAMOS EL ID (NUMBER), NO EL OBJETO
