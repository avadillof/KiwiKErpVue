...
export default defineComponent({
    ...
    setup() {
        ...
        function verPerfilUsuario(): void {
            ...
            // CORRECCIÓN: Extrae solo el pkid del usuario
            if (userFormRef.value && authStore.user && authStore.user.pkid) {
                const idParaEnviar = Number(authStore.user.pkid); // Nos aseguramos que sea un número
                console.log("Enviando ID al formulario:", idParaEnviar);
                userFormRef.value.open(idParaEnviar);
            } else {
                console.error("No se puede abrir el perfil: usuario o pkid no encontrados.");
            }
        }
