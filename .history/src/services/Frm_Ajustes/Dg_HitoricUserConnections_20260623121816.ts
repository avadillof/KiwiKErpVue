...
export function useDgHistoricUserConnectionsController() {
    ...
    const applyAllFilters = () => {
        ...
        if (fechaFiltroByDate.value?.[0] && fechaFiltroByDate.value?.[1]) {

            const [inicio, fin] = fechaFiltroByDate.value;
    ...
    watch(fechaFiltroByDate, (nuevoValor) => {
            nextTick(() => {
                applyAllFilters();
...
watch([fechaFiltroByDate, isClientFilterValue,isProveFilterValue], () => {
    applyAllFilters();
});
