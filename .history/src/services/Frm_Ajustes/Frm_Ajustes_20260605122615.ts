...
export function Frm_Ajustes() {
    ...
    async function saveData() {
        ...
        try {
            ...
            toast.add({ severity: 'success', summary: 'Guardat', detail: 'Dades actualitzades', life: 3000 });
        } catch (error) {
            toast.add({ severity: 'error', summary: 'Error', detail: 'No es va poder guardar', life: 3000 });
        } finally {
            loading.value = false;
        }
    }
    ...
    function onUploadLogo(event: any) {
        ...
        logoUrl.value = `${assetsUrl}/base/logo.png?t=${new Date().getTime()}`;
        formData.value.urlLogoCompany = logoUrl.value;
        toast.add({ severity: 'success', summary: 'Logo actualitzat', detail: 'El logo s\'ha pujat correctament.', life: 3000 });
    }

    return {
        formData, loading, saveData, loadData, provincias, onUploadLogo, logoUrl,
    };
}