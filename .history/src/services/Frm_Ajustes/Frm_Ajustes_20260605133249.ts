...
import { useCompanyStore } from '@/stores/companyStore'; // Ruta de tu store
...
export function Frm_Ajustes() {
    ...
    const formData = computed(() => companyStore.companyInfo);
    ...
    async function saveData() {
        ...
        if (!formData.value.nameCompany?.trim() || !formData.value.cifCompany?.trim()) {
            toast.add({
                severity: 'warn',
                summary: 'Validación',
                detail: 'El nombre de la entidad y el CIF son obligatorios',
                life: 3000
            });
        ...
        if (formData.value.email && formData.value.email.trim() !== '' && HelperString.isValidEmail(formData.value.email) == false) {
        ...
        if (formData.value.cifCompany && formData.value.cifCompany.trim() !== '' && HelperString.isValidCifNif(formData.value.cifCompany) == false) {

            toast.add({
                severity: 'warn',
                summary: 'Validación',
                detail: 'El Cif/NIf no es válido',
                life: 3000
            });
            return;
        }
    ...
    async function onUploadLogo(event: any) {
        ...
        try {
            ...
            companyStore.updateLogoUrl(logoUrl.value);
