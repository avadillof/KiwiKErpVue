...
export function useDgHistoricUserConnectionsController() {
    ...
    watch(fechaFiltro, (nuevoValor) => {
    if (nuevoValor && nuevoValor[0] && nuevoValor[1]) {
        // En lugar de pasar un string extraño, actualiza el estado de la tabla
        // Si tu GenericDataTable admite una función de refresh, pásale los datos limpios
        nextTick(() => {
            if (tableRef.value && typeof tableRef.value.refresh === 'function') {
                // Opción recomendada: usar el mecanismo de filtros del componente
                tableRef.value.refresh(); 
            }
        });
        }
