...
export function Frm_Ajustes() {
    ...
    async function saveData() {
        ...
        if (formData.value.email && !HelperString.isValidEmail(formData.value.email)) {
            toast.add({ severity: 'warn', summary: 'Validación', detail: 'El formato del email Corporativo de la Empresa no es válido', life: 3000 });
            return;
        }

        if (formData.value.mailSender && !HelperString.isValidEmail(formData.value.mailSender)) {
            toast.add({ severity: 'warn', summary: 'Validación', detail: 'El formato del email de la cuenta del Sistema no es válido', life: 3000 });
            return;
        }
