...
export function loginController() {
    ...
    async function handleLogin() {
        ...
        // Opción: esperar brevemente o bloquear
            await init(); 
        }

        console.log("Valor actual de duration:", companyStore.companyInfo.toastDuration);
        console.log("Tipo de dato:", typeof companyStore.companyInfo.toastDuration);

        // 1. Validamos campos
        if (!loginData.value.username || !loginData.value.password) {
            console.info(companyStore.companyInfo);

            toast.add({ severity: 'error', summary: 'Campos incompletos', detail: 'Usuario y contraseña son requeridos.', life: companyStore.companyInfo.toastDuration ?? 3000 });
            return;
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                toast.add({ severity: 'error', summary: 'Error', detail: 'Usuario o contraseña inválidos.', life: companyStore.companyInfo.toastDuration ?? 3000 });
            } else {
            ...
            if (err.name === 'AbortError') {
                ...
                toast.add({ severity: 'error', summary: 'Error de conexión', detail: 'Tiempo de espera agotado.', life: companyStore.companyInfo.toastDuration ?? 3000 });
            } else {
                console.error(err);
                errorMessage.value = "Error de conexión con el servidor.";
                toast.add({ severity: 'error', summary: 'Error', detail: 'No se pudo conectar al servidor.', life: companyStore.companyInfo.toastDuration ?? 3000 });
            }
        } finally {
