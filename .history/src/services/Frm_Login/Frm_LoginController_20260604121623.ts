...
const companyStore = useCompanyStore();
    ...
    async function handleLogin() {
        ...
        try {
            ...
            // 2. Construimos el cuerpo usando las propiedades exactas de tu UserDTO
            const loginPayload = {
                name: loginData.value.username,     // Adaptado: de codeUser a name
                password: loginData.value.password
            };

            // Realizamos la petición por POST enviando el JSON en el cuerpo
            ...
            if (response.ok) {
                // Consumimos el stream una única vez y guardamos en una constante
