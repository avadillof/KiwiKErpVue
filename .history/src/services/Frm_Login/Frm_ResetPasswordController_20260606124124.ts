...
export function useForgotPasswordController(isVisibleRef: Ref<boolean>) {
    ...
    async function handleRequestReset() {
        ...
        try {
            ...
            if (response.ok) {
                toast.add({ severity: 'success', summary: 'Éxito', detail: 'Instrucciones enviadas correctamente', companyStore.companyInfo.toastDuration ?? 3000 });
                isVisibleRef.value = false;
                
            } else {
                toast.add({ severity: 'error', summary: 'Error', detail: 'No se pudo procesar la solicitud', companyStore.companyInfo.toastDuration ?? 3000 });
            }
        } catch (error) {
