...
export function loginController() {
    ...
    async function handleLogin() {
        ...
        try {
            ...
            // 2. Construimos el cuerpo del mensaje en formato JSON de forma segura
            const loginPayload = {
                codeUser: loginData.value.username,
                password: loginData.value.password
            };

            // Realizamos la petición por POST, ocultando los datos en el body
            const response = await fetch(`${baseUrl}/WebLoginUser`, {
                method: 'POST', // Cambiado a POST
                headers: {
                    'Content-Type': 'application/json' // Indicamos que el cuerpo es un JSON
                },
                body: JSON.stringify(loginPayload) // Convertimos el objeto JS a cadena de texto JSON
            });

            // 3. Procesamos la respuesta
            if (response.ok) {
                // 1. Consumimos el stream una única vez y guardamos en una constante
                const userDTO = await response.json();

                // 2. Usamos esa constante para todo lo que necesites
