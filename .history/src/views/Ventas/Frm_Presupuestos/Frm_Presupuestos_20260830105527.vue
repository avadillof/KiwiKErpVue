    ...
    <div class="sales-page">
        ...
        <div class="card list-card">
            ...
            <GenericDataTable class="quote-table" ref="tableRef" dataKey="pkid" selectionMode="single" v-model:selection="selectedQuote"
                ...
                <Column field="code" sortField="salesOrdersDsCode" header="Código" sortable style="width:10%" bodyClass="nowrap-cell"><template #body="{ data }"><span class="code-with-attachments"><span :class="{ 'draft-code': isDraftEstimation(data.state) || data.code?.startsWith('DRAFT_') }">{{ data.code }}</span><Tag v-if="Number(data.attachmentCount || 0) > 0" :value="String(data.attachmentCount)" icon="pi pi-paperclip" severity="info" rounded title="Documentos adjuntos" /><i v-if="data.hasNotes" class="pi pi-comment notes-indicator" title="Tiene observaciones"></i></span></template></Column>
                <Column field="entityName" sortField="entitie.entitieDsName" header="Cliente" sortable style="width: 14%" />
