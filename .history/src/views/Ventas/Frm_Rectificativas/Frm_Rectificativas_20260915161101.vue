<template>
  ...
  <main class="rec-page">
    <header class="hero">
      ...
      <Button label="Ventas" icon="pi pi-arrow-left" text severity="secondary"
    ...
    <section class="list-card">
      <div class="toolbar">
        <div>
          ...
          <small>Los borradores reservan las cantidades que se van a abonar.</small>
        </div>
        <Button class="corporate" label="Nueva rectificativa" icon="pi pi-plus" @click="openNew" />
      </div>
      <form class="filters" @submit.prevent="load(0)">
        <InputText v-model="query" placeholder="Código, referencia o cliente" aria-label="Buscar rectificativa" />
        <Select v-model="status" :options="statuses" optionLabel="label" optionValue="value"
          placeholder="Todas las situaciones" showClear aria-label="Situación" @change="load(0)" /><Button
          label="Buscar" icon="pi pi-search" type="submit" :loading="loading" /><Button label="Actualizar"
          icon="pi pi-refresh" text :disabled="loading" @click="refresh" />
      </form>
      <GenericDataTable class="table compact-table custom-header-table" selectionMode="single"
        v-model:selection="selected" dataKey="pkid" endpoint="WebGetSalesRecInvoices"
        :params="{ status: status || undefined }" :requestConfig="() => auth.portalRequestConfig()"
        :showPaginator="true" :filterable="true" :showActions="false">
        <template #empty>No hay rectificativas para estos filtros.</template>
        <Column field="code" header="Código" sortable style="min-width: 155px" bodyStyle="white-space:nowrap"><template
            #body="{ data }"><span class="code-with-indicators"><span :class="{ 'draft-code': data.draft }">{{
              data.code
                }}</span><span v-if="data.rectificationCount" class="rectification-indicator"
                :title="rectificationTooltip(data)">
                <Tag :value="String(data.rectificationCount)" icon="pi pi-undo" severity="warn" rounded />
              </span></span></template>
        </Column>
        <Column field="date" header="Fecha" sortable style="width: 8rem"
          bodyStyle="white-space:nowrap;font-variant-numeric:tabular-nums"><template #body="{ data }"><span
              class="invoice-date-time">{{
                dateTime(data.date)
              }}</span></template></Column>
        <Column field="customer" header="Cliente" sortable style="min-width: 180px" />
        <Column field="sourceCode" header="Documento rectificado" sortable />
        <Column field="state" header="Situación" sortable><template #body="{ data }">
            <Tag :value="stateLabel(data.state)" :severity="data.draft ? 'warn' : data.cancelled ? 'secondary' : 'success'
              " />
          </template></Column>
        <Column field="verifactuStatus" header="VeriFactu" sortable><template #body="{ data }">
            <Tag v-if="data.verifactuStatus" :value="fiscalLabel(data.verifactuStatus)" :severity="data.verifactuStatus === 'ACCEPTED' ? 'success' : 'warn'
              " /><span v-else>—</span>
          </template></Column>
        <Column field="total" header="Total" sortable style="width: 10%; text-align: right"
          bodyStyle="text-align:right;white-space:nowrap;font-variant-numeric:tabular-nums"><template
            #body="{ data }">{{
              rectMoney(data.total, data.currency)
            }}</template></Column>
        <Column header="" style="width: 4rem"><template #body="{ data }"><Button icon="pi pi-ellipsis-v" text rounded
              aria-label="Acciones de la rectificativa" :disabled="busy" @click="openMenu($event, data)" /></template>
        </Column>
      </GenericDataTable>
      <Menu ref="rowMenu" :model="rowItems" popup />
    </section>

    <Dialog v-model:visible="pickerVisible" modal header="Nueva rectificativa · Documento de origen"
      class="rec-dialog invoice-picker-dialog" :style="{ width: '65rem', height: '70vh' }"
      ...
      <form class="filters" @submit.prevent="findInvoices">
        <InputText v-model="invoiceQuery" placeholder="Código o cliente de la factura o rectificativa"
          aria-label="Buscar documento de origen" /><Button label="Buscar" icon="pi pi-search" type="submit"
          :loading="busy" />
      </form>
      <DataTable :value="invoiceOptions" :loading="busy" dataKey="selectionKey" scrollable scrollHeight="45vh"><template
          #empty>Busca una factura o rectificativa aceptada.</template>
        <Column field="code" header="Documento" />
        <Column field="sourceType" header="Tipo" />
        <Column field="customer" header="Cliente" />
        <Column header="Acciones"><template #body="{ data }"><Button label="Seleccionar" :disabled="busy" @click="
      ...
      <template #footer>
        <div class="footer">
          ...
          <Button label="Cerrar" severity="secondary" text @click="pickerVisible = false" />
        </div>
      </template>
    </Dialog>

    <Dialog v-model:visible="detailVisible" modal
      :header="form.pkid ? `Rectificativa ${form.code}` : 'Nueva rectificativa'" class="rec-dialog"
      :style="{ width: '78rem' }" :breakpoints="{ '1100px': '96vw' }" :closable="!busy" :closeOnEscape="!busy">
      <Message v-if="dialogError" severity="error" :closable="false">{{
        dialogError
        }}</Message>
      <div class="origin">
        ...
        <div>
          <small>Factura original</small><Button :label="form.invoiceCode" link @click="
            router.push({
              name: 'Facturas',
      ...
      <Message v-if="editable && form.mode === 'QUANTITY'" severity="info" :closable="false">{{
      ...
      <Message v-if="editable && form.mode === 'ECONOMIC'" severity="warn" :closable="false">Rectificación económica:
        indica
        el nuevo precio, descuento, incremento
        o impuesto. El PDF mostrará la diferencia frente a la factura
        original.</Message>
      <div class="form-grid">
        <label>Modalidad<Select v-model="form.mode" :options="modes" optionLabel="label" optionValue="value"
            :disabled="!!form.pkid || busy" /></label>
        <label>Fecha
          <DatePicker v-model="form.dateValue" dateFormat="dd/mm/yy" showIcon :disabled="!editable || busy"
            :maxDate="new Date()" />
        </label>
        <label>Referencia
          <InputText v-model="form.reference" :disabled="!editable || busy" maxlength="145" />
        </label>
        <label class="wide">Motivo de rectificación *
          <InputText v-model="form.reason" :disabled="!editable || busy" maxlength="250" />
        </label>
      </div>
      <DataTable :value="form.lines" dataKey="sourceLineId" scrollable class="lines-table">
        <Column field="description" header="Descripción" style="min-width: 14rem" />
        <Column v-if="editable" field="available" header="Disponible" />
        <Column header="Cantidad"><template #body="{ data }">
            <InputNumber v-if="editable" v-model="data.quantity" :disabled="busy" :min="0" :max="data.available"
              :maxFractionDigits="3" locale="de-DE" :useGrouping="true" class="quantity"
              aria-label="Cantidad a rectificar" /><span v-else>{{ numeric(data.quantity) }}</span>
          </template></Column>
        <Column header="Precio"><template #body="{ data }">
            <InputNumber v-if="editable && form.mode === 'ECONOMIC'" v-model="data.priceUnit" :min="0"
              :maxFractionDigits="4" locale="de-DE" :useGrouping="true" class="quantity" /><span v-else>{{
                money(data.priceUnit, form.currency) }}</span><small v-if="editable && form.mode === 'ECONOMIC'">Original:
              {{ money(data.originalPriceUnit, form.currency) }}</small>
          </template></Column>
        <Column header="Dto."><template #body="{ data }">
            <InputNumber v-if="editable && form.mode === 'ECONOMIC'" v-model="data.discount" :min="0" :max="100"
              :maxFractionDigits="2" locale="de-DE" suffix=" %" class="quantity" /><span v-else>{{
                numeric(data.discount) }} %</span>
          </template></Column>
        <Column header="Incremento"><template #body="{ data }">
            <InputNumber v-if="editable && form.mode === 'ECONOMIC'" v-model="data.increment" :min="0" :max="100"
              :maxFractionDigits="2" locale="de-DE" suffix=" %" class="quantity" /><span v-else>{{
                numeric(data.increment) }} %</span>
          </template></Column>
        <Column header="IVA"><template #body="{ data }">
            <InputNumber v-if="editable && form.mode === 'ECONOMIC'" v-model="data.tax" :min="0" :max="100"
              :maxFractionDigits="2" locale="de-DE" suffix=" %" class="quantity" /><span v-else>{{ numeric(data.tax) }}
              %</span>
          </template></Column>
        <Column header="Total"><template #body="{ data }">{{
      ...
      <div class="form-grid">
        <label class="wide">Condiciones<Textarea v-model="form.terms" rows="2" maxlength="1000"
            :disabled="!editable || busy" /></label><label class="wide">Notas internas<Textarea v-model="form.notes"
            rows="2" :disabled="!editable || busy" /></label>
      </div>
      ...
      <template #footer>
        <div class="footer">
          <div class="kiwik-separator" />
          <div class="actions">
            <Button label="Cerrar" text severity="secondary" :disabled="busy" @click="detailVisible = false" /><Button
              v-if="form.pkid" label="PDF" icon="pi pi-file-pdf" outlined :disabled="busy"
              @click="downloadPdf" /><Button v-if="form.pkid && editable" label="Cancelar borrador" severity="danger"
              text :disabled="busy" @click="cancelVisible = true" /><Button v-if="editable" label="Guardar borrador"
              :disabled="!hasSelectedLines" icon="pi pi-save" class="corporate" :loading="busy" @click="save" /><Button
              v-if="form.pkid && editable" label="Guardar y emitir" icon="pi pi-check"
              :disabled="busy || !hasSelectedLines" @click="issueVisible = true" /><Button
              v-if="form.verifactuStatus === 'REJECTED'" label="Reintentar VeriFactu" :disabled="busy"
              @click="issueVisible = true" /><Button v-if="form.pkid && !editable" label="Actualizar estado"
              icon="pi pi-refresh" text :disabled="busy" @click="openDetail(form.pkid)" />
          </div>
        </div>
      </template>
    </Dialog>

    <Dialog v-model:visible="issueVisible" modal :closable="!busy" :closeOnEscape="!busy"
      :style="{ width: 'min(520px,94vw)' }" class="kiwik-dialog issue-invoice-dialog" @show="validateIssueTarget" :pt="{
      ...
      <Message severity="warn" :closable="false">
        {{
          editable
            ? "Se asignará el número fiscal y se bloqueará la edición. El envío a VeriFactu se procesará según los Ajustes
        de
        Ventas."
      ...
      <Message v-if="validatingIssue" severity="info" :closable="false">Comprobando datos fiscales y económicos…
      </Message>
      <Message v-else-if="issueValidationErrors.length" severity="error" :closable="false">
        <b>No se puede emitir. Corrige estos datos:</b>
        <ul>
          <li v-for="item in issueValidationErrors" :key="item">{{ item }}</li>
        </ul>
        <Button label="Volver a comprobar" icon="pi pi-refresh" text @click="validateIssueTarget" />
      </Message>
      <Message v-else-if="issueValidated && editable" severity="success" :closable="false">Datos comprobados. Al emitir
        se
        validarán de nuevo antes de asignar
        número. Esta comprobación no acredita el alta censal ni VIES.</Message>
      <label v-if="!validatingIssue && !issueValidationErrors.length" class="verifactu-password">
        <span>Contraseña del certificado VeriFactu</span>
        <Password v-model="password" :disabled="busy" toggleMask :feedback="false" fluid autofocus />
      </label>
      <template #footer>
        <div class="issue-dialog-footer">
          <div class="issue-dialog-separator"></div>
          <div class="issue-dialog-actions">
            <Button label="Volver" text severity="secondary" :disabled="busy" @click="issueVisible = false" />
            <Button :label="editable ? 'Emitir y enviar' : 'Reintentar envío'" icon="pi pi-send" severity="success"
    ...
    <Dialog v-model:visible="cancelVisible" modal header="Cancelar borrador" class="rec-dialog" :closable="!busy">
      <p>
      ...
      <template #footer>
        <div class="footer">
          <div class="kiwik-separator" />
          <Button label="Volver" text :disabled="busy" @click="cancelVisible = false" /><Button
            label="Cancelar borrador" severity="danger" :loading="busy" @click="cancelDraft" />
        </div>
      </template>
    </Dialog>
...
.list-card .toolbar h2 {
  margin: 0 0 3px;
  font-size: 1rem;
}

.list-card .toolbar small {
  font-size: .75rem;
}
...
.list-card :deep(.p-datatable-wrapper) {
  max-width: 100%;
  min-width: 0;
}

.list-card :deep(.p-datatable-table) {
  min-width: 0;
}
