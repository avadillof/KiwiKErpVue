     ...
     <Dialog v-model:visible="visible" modal :header="entity.pkid > 0 ? 'Editar Entidad' : 'Nueva Entidad'"
        ...
        <div class="kiwik-form-container">
            <div class="grid p-fluid mb-4">
                ...
                <div class="col-12 md:col-6">
                    <FloatLabel variant="on">
                        <InputText id="dsCif" v-model="entity.dsCif" />
                        <label for="dsCif">NIF / CIF</label>
                    </FloatLabel>
                </div>

                <div class="col-12 md:col-6 flex gap-3 align-items-center">
                    <div class="flex align-items-center">
                        <Checkbox v-model="entity.bolClient" binary inputId="c_client" />
                        <label for="c_client" class="ml-2">Es Cliente</label>
                    </div>
                    <div class="flex align-items-center">
                        <Checkbox v-model="entity.bolSupplier" binary inputId="c_supplier" />
                        <label for="c_supplier" class="ml-2">Es Proveedor</label>
                    </div>
                </div>
            </div>
        </div>
...
const generateCode = async () => {
    try {
        ...
        entity.value.userDsCode = res.data;
    } catch (error) {
        toast.add({
            severity: 'error',
            summary: 'Error',
