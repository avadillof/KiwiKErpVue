...
export function useFormValidator<T extends Record<string, any>>(initialErrors: T) {
    const errors = ref<T>(initialErrors);
    const refs = ref<Record<string, any>>({});

    // Tipamos 'el' como 'any' para evitar conflictos con componentes de PrimeVue
