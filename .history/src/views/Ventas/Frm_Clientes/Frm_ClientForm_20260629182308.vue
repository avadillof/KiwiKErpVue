...
const validate = () => {
    ...
    if (!entity.value.web?.trim()) {
        ...
        if(HelperString.isValidUrl(entity.value.web)==false){
            errors.value.web = 'El campo Web tiene un valor no válid';
            valid = false;
        }
    }

    if (!entity.value.mail?.trim()) {
        valid = true;
    }else{
        if(HelperString.isValidEmail(entity.value.mail)==false){
            errors.value.web = 'El campo Mail tiene un valor no válido';
            valid = false;
        }
    }
