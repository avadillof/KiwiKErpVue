            ...
            <Panel>
                ...
                <div class="grid p-fluid mb-12">
                    ...
                    <div class="col-12 md:col-6">
                        <FloatLabel variant="on" class="w-full">
                            <InputText :ref="(code) => setRef('code', code) " maxlength="120" id="dsName" v-model="entity.name" class="w-full" required />
...
const validate = () => {
    ...
    if (entity.value.dateUp == null) {
        errors.value.fechaalta = 'La Fecha de Alta es obligatorio';
        valid = false;
    }
...
export function useFormValidator(initialErrors: Record<string, string>) {
    ...
    const scrollToError = async (errorKey: string) => {
        ...
        if (element) {
            // Accedemos al elemento DOM del componente PrimeVue
            const el = element.$el || element;
            el.scrollIntoView({ behavior: 'smooth', block: 'center' });
            
            // Si tiene un input interno, intentamos enfocarlo
            const input = el.querySelector('input') || el;
            input.focus?.();
        }
    };
    return { errors, refs, setRef, scrollToError };
}


defineExpose({setRef, open });
</script>
