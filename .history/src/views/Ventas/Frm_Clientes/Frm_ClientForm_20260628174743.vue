...
const open = (data: any) => {
    if (data) {
        // Mapeamos el DTO del Back a nuestro objeto del Form
        entity.value = {
            pkid: data.pkid,
            dsCode: data.code,
            dsName: data.name,
            dsCif: data.cif,
            bolClient: data.isclient,
            bolSupplier: data.isprove,
            isActive: data.active,
            dateUp: data.dateUp,
            salesAttributes: { ... }, // Aquí cargaremos atributos después
            purchasesAttributes: { ... }
        };
    } else {
        // Inicialización para nuevo cliente
        entity.value = {
            pkid: null,
            dsName: '',
            dsCif: '',
