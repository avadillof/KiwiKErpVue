...
const handledAfterSearch = (event: any) => {
    ...
    if (tableRef.value) {
        (tableRef.value as any).searchQuery = ''; 
    }
...
watch([fechaFilterdateUp, isClientFilterValue], () => {
    applyAllFilters('filter');
});

// Esta función ahora controla todo el estado de filtrado
const applyAllFilters = (source: 'search' | 'filter') => {

    // Si viene de una búsqueda de texto, limpiamos filtros específicos
    if (source === 'search') {
        fechaFilterdateUp.value = null;
        isClientFilterValue.value = null;
    }
    // Si viene de un filtro (fecha/booleano), limpiamos el buscador
    ...
    // Incluimos el texto de búsqueda si no está vacío
    if ((tableRef.value as any).searchQuery.value.trim() !== '') {
        queryParts.push((tableRef.value as any).searchQuery = ''.value.trim());
    }
