<template>
    <div style="background-color: #f9fafb; min-height: 100vh; padding: 20px; margin-bottom: 100px;">
                ...
                <Column field="dateUp" sortable style="width: 15%">
                                ...
                                <template #header>
                                    <div class="flex align-items-center justify-content-between w-full">
                                        ...
                                        <Button  :severity="fechaFiltroByDate ? 'danger' : 'secondary'"  icon="pi pi-filter"  rounded size="small" @click.stop="onFilterByDateClick($event) " />
                                    </div>
                                </template>


                                <template #body="slotProps">
                                    {{ HelperDates.formatDateFromLocale((slotProps.data?.dateUp)) }}
                                </template>
        ...
        <OverlayPanel ref="opDateFilterByDate" appendTo="body" >
            <DateRangePicker  v-model="fechaFiltroByDate" />
        </OverlayPanel>
...
const fechaFiltroByDate = ref<[Date | null, Date | null] | null>(null);
