<template>
    ...
    <template #header>
        <div class="flex align-items-center justify-content-between w-full">
            ...
            <Button 
                :severity="isClientFilterValue !== null ? 'danger' : 'secondary'" 
                icon="pi pi-filter" 
                rounded
                size="small" 
                @click.stop="opIsclientFilterDateUp($event)" 
            />
        </div>
    </template>
    <template #body="slotProps">
        ...
        <OverlayPanel ref="FrmFilterIsClient" appendTo="body">
            <div class="flex flex-column gap-2 p-2">
                ...
                <Button label="Solo Clientes" text size="small"
                    @click="isClientFilterValue = true; FrmFilterIsClient.hide()" />
                <Button label="Solo No Clientes" text size="small"
