<template>
    <div style="background-color: #f9fafb; min-height: 100vh; padding: 20px; margin-bottom: 100px;">
                ...
                <Column field="isClient"  style="width: 8%">
                   ...
                   <template #header>
                        <div class="flex align-items-center justify-content-between w-full">
                            ...
                            <Button :severity="fechaFilterdateUp ? 'danger' : 'secondary'" icon="pi pi-filter" rounded
                                size="small" @click.stop="opIsclientFilterDateUp($event)" />
                        </div>
                    </template>
        ...
        <OverlayPanel ref="FrmFilterIsClient" appendTo="body">
    <div class="flex flex-column gap-2 p-2">
        <span class="text-sm font-medium">Filtrar por tipo</span>
        <Button label="Todos" text size="small" @click="isClientFilterValue = null; FrmFilterIsClient.hide()" />
        <Button label="Solo Clientes" text size="small" @click="isClientFilterValue = true; FrmFilterIsClient.hide()" />
        <Button label="Solo No Clientes" text size="small" @click="isClientFilterValue = false; FrmFilterIsClient.hide()" />
    </div>
</OverlayPanel>
