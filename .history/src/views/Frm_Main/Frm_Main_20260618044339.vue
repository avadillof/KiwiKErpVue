...
export default defineComponent({
  ...
  components: { 
    Avatar, 
    OverlayBadge,
    // Carga el formulario de forma asíncrona para evitar el error de tipos
    Frm_UserForm: defineAsyncComponent(() => import('../../views/Frm_Main/Frm_Ajustes/Frm_UserForm.vue'))
  },
  setup(props, context) {
    const userFormRef = ref<any>(null); // Usamos any para evitar que TS valide la estructura interna
    const controller = Frm_Main as any;

    const setupResult = controller.setup ? controller.setup(props, context) : {};

    // Inyectamos la referencia al controlador
    if (setupResult) {
