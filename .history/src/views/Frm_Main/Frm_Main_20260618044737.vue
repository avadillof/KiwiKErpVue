...
<template>
  <Frm_UserForm ref="userFormRef" @saved="onUserUpdated" />  
</template>


<script lang="ts">
import { defineComponent, type ComponentOptions, ref, defineAsyncComponent } from 'vue';
...
export default defineComponent({
  ...
  components: { 
    Avatar, 
    OverlayBadge,
    // Registramos el componente únicamente aquí de forma asíncrona
    Frm_UserForm: defineAsyncComponent(() => import('../../views/Frm_Main/Frm_Ajustes/Frm_UserForm.vue'))
  },
  setup(props, context) {
    // 1. Definimos la ref en la vista
    ...
    // 3. Inyectamos la referencia al resultado del controlador
    // Esto permite que el controlador use 'userFormRef'
    setupResult.userFormRef = userFormRef;

    // 4. Retornamos todo fusionado para que el template acceda a ambos
    return {
      ...setupResult,
      userFormRef
    };
