<template>
  <div style="padding: 40px; font-family: 'Segoe UI', Arial, sans-serif; background-color: #f9fafb; min-height: 100vh;">
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(260px, 1fr)); gap: 25px; margin-bottom: 40px;">
      
      <p-card 
        v-for="modulo in modulosVisibles" 
        :key="modulo.id"
        style="border: 1px solid #e5e7eb; border-radius: 12px; cursor: pointer; overflow: hidden; transition: transform 0.2s, box-shadow 0.2s;"
        class="p-shadow-1"
    ...
    <p-divider align="left" style="margin: 40px 0 25px 0;">
...
<script lang="ts">
// Vinculación estricta con el controlador TypeScript externo
import Frm_MainController from '../../services/Frm_Main/Frm_Main';
export default Frm_MainController;
</script>