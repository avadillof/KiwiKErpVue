...
<script lang="ts">
import { defineComponent, type ComponentOptions, ref, onMounted } from 'vue'; 
...
export default defineComponent({
  ...
  setup(props, context) {
    // Definimos la ref aquí
    const userFormRef = ref<any>(null); // Usar 'any' evita el error 4023
    const controller = Frm_Main as any;

    // Ejecutamos el setup y capturamos su resultado
    const setupResult = controller.setup ? controller.setup(props, context) : {};

    // Inyectamos la referencia al objeto que el controlador necesita
    if (setupResult) {
