...
export default defineComponent({
  name: 'Frm_MainLayout',
  components: { Avatar, OverlayBadge, Frm_UserForm },
  setup(props, context) {
    // 1. Creamos la ref aquí en la vista
    const userFormRef = ref();
    
    // 2. Ejecutamos el setup del controlador
    const controller = Frm_Main as any;
    const controllerSetup = controller.setup ? controller.setup(props, context) : {};

    // 3. Inyectamos la ref al controlador para que él pueda usarla
    if (controllerSetup.userFormRef) {
        controllerSetup.userFormRef.value = userFormRef.value;
    }

    // 4. Retornamos todo fusionado
    return {
      ...controllerSetup,
      userFormRef
    };
