<template>
    <Dialog v-model:visible="visible" modal :header="formData.pkid > 0 ? 'Editar Usuario' : 'Nuevo Usuario'"
        ...
        <div class="flex flex-column h-full">
            ...
            <div class="kiwik-form-container overflow-y-auto pb-4" style="max-height: 70vh;">
                <div class="flex flex-column align-items-center gap-2 mb-3">
                    <div class="relative">
                        <div class="border-circle overflow-hidden border-3 border-primary shadow-2 flex align-items-center justify-content-center"
                ...
                <div class="flex flex-column gap-4">
                    <InputGroup>
                        ...
                        <Button icon="pi pi-id-card" @click="generateCode" :disabled="formData.pkid > 0"
                            severity="secondary" v-tooltip.top="'Generar código'" />
                    </InputGroup>
                    ...
                    <div class="w-full">
                        ...
                        <Select v-model="formData.groupKyId" :options="groupOptions" optionLabel="label"
                            optionValue="value" class="w-full" placeholder="Seleccionar..." />
                    </div>

                    <FloatLabel variant="on" class="w-full">
                        <Password id="user_pass" v-model="formData.password" class="w-full" toggleMask
                            :feedback="false" />
            ...
            <div class="mt-4 pt-3 flex justify-content-end" style="border-top: 2px solid var(--primary-color);">
                <Button label="Guardar usuario" icon="pi pi-check" @click="save" style="width: 150px;" />
            </div>
        </div>
    </Dialog>
...
/* Aseguramos que los inputs se vean bien en el flex */
.p-float-label {
    width: 100%;
}
</style>
