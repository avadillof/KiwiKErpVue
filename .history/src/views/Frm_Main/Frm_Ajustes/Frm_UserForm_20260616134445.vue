        ...
        <div class="kiwik-form-container">
            ...
            <div class="flex flex-column gap-4 mt-3">
                ...
                <InputGroup>
                    ...
                    <Button 
                        icon="pi pi-id-card" 
                        @click="generateCode" :disabled="formData.pkid > 0"
                        severity="secondary" 
                ...
                <div v-if="formData.pkid > 0" class="w-full">
    <label class="text-sm text-gray-500">Fecha de Alta</label>
    <InputText 
        :value="formData.userDtDateUp ? new Date(formData.userDtDateUp).toLocaleDateString() : 'N/A'" 
        class="w-full" 
        disabled 
    />
</div>
    ...
    try {
        ...
        if (user) {
            formData.value = {
                ...
                active: user.active,
                pin: user.pin || '',
                userDtDateUp: user.userDtDateUp
            };
        } else {
            // Reset limpio
            formData.value = {
...
const save = async () => {
    ...
    if (!formData.value.userDsCode.trim() ||
        ...
        toast.add({
            ...
            detail: 'El código, nombre, email y rol son obligatorios.',
            life: 3000
        });
        return;
    }
