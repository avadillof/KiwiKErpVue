<template>
    <Dialog v-model:visible="visible" modal :header="formData.pkid > 0 ? 'Editar Usuario' : 'Nuevo Usuario'"
        ...
        <div class="kiwik-form-container grid-layout">
            ...
            <div class="col-left flex flex-column align-items-center justify-content-start pt-3">
                <div class="relative">
                    ...
                    <Button icon="pi pi-camera" rounded class="absolute bottom-0 right-0 p-2 flex align-items-center justify-content-center" 
                        @click="openCamera" severity="primary" style="width: 36px; height: 36px;" />
                </div>
            </div>

            <!-- Columna Derecha: Formulario -->
            <div class="col-right flex flex-column gap-3">
                <InputGroup>
        ...
        <!-- Secciones adicionales que ocupan ancho completo debajo -->
        <div class="mt-4 flex flex-column gap-3 px-3">
            ...
            <Checkbox v-model="formData.active" :binary="true" inputId="active_check" label="Usuario Activo" />
        </div>
    ...
    <!-- ... tu dialog de cámara y toast siguen igual ... -->
</template>

<style scoped>
.grid-layout {
    display: grid;
    grid-template-columns: 140px 1fr; /* Columna izquierda fija, derecha flexible */
    gap: 1.5rem;
    align-items: start;
}
