<template>
  ...
  <div class="p-4" style="padding-bottom: 80px;">
    ...
    <img v-if="formData.urlLogoCompany" :src="formData.urlLogoCompany" style="max-height: 120px; object-fit: contain;" />
    <i v-else class="pi pi-image text-5xl text-400 mb-3"></i>

    <FileUpload 
        mode="basic" 
        name="logo" 
        :url="`${assetsUrl}/upload-logo`" 
        accept="image/*" 
        :maxFileSize="1000000" 
        @upload="onUploadLogo" 
...
const { formData, loading, saveData, provincias, loadData } = Frm_Ajustes();
