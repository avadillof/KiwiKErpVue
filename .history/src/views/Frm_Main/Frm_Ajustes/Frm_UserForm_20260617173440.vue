<template>
    ...
    <Dialog
        ...
        @hide="stopCamera"
    >
        <div class="flex flex-column" v-if="cameraVisible">
            <video
                ref="videoRef"
                autoplay
                playsinline
                class="w-full border-round shadow-2"
                style="background: #000; min-height: 250px;"
            ></video>
...
const companyStore = useCompanyStore(); // eslint-disable-line @typescript-eslint/no-unused-vars
    ...
    try {
        ...
        if (user) {
            ...
            formData.value = {
                ...
                groupKyId: Number(user.groupKyId), // Ensure it's a number
...
const save = async () => {
    ...
    const dataToSend = {
        ...
        userDtDateUp: formData.value.userDtDateUp
            ? new Date(formData.value.userDtDateUp).toISOString()
            : null
    };
