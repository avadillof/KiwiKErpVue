...
async function loadContact(id: number) {
    ...
    try {
        ...
        contact.value.departamentos.forEach((dep: any) => {

            const seleccionado = data.departamentos.find(
                (d: any) => d.departamentoPkid === dep.departamentoPkid
            );

            dep.selected = !!seleccionado;
            dep.mailCopy = seleccionado?.mailCopy ?? false;
