                ...
                <div class="grid p-fluid mb-12">
                    ...
                    <div class="col-12 md:col-2">
                        ...
                        <div class="flex align-items-center gap-2 mt-2">
                            ...
                            <Checkbox id="active" v-model="productFamily.active" binary />
...
const open = async (pkid) => { // Limpiar formulario productFamily.value = { pkid: null, code: '', description: '', descriptionEn: '', active: true, inventariable: false }; errors.value = { code: '', description: '', descriptionEn: '' }; visible.value = true; // ===================================================== // NUEVO // ===================================================== if (!pkid) { return; } // ===================================================== // EDICIÓN // ===================================================== try { const response = await axios.get( `${import.meta.env.VITE_API_URL}/WebGetProductFamily`, { params: { pkid: pkid } } ); productFamily.value = { pkid: response.data.pkid, code: response.data.code ?? '', description: response.data.description ?? '', descriptionEn: response.data.descriptionEn ?? '', active: response.data.active ?? false, inventariable: response.data.inventariable ?? false }; } catch (error) { console.error(error); visible.value = false; toast.add({ severity: 'error', summary: 'Error', detail: error.response?.data ?? 'No se pudo cargar la Familia de Producto', life: companyStore.companyInfo.toastDuration ?? 3000 }); } };
