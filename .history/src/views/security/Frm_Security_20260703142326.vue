<template>
    <div class="p-4 h-screen flex flex-column" style="padding-bottom: 80px;">
        ...
        <Message variant="simple" icon="pi pi-info-circle" severity="success" class="mb-4">
            Configure la securización y accesos a KiwiKERP para el Usuario seleccionado.
        </Message>
        ...
        <div class="card p-3 shadow-sm border-1 surface-border border-round-md mb-4 flex align-items-center gap-3">
            <div class="bg-primary-100 p-2 border-circle">
                <i class="pi pi-shield text-primary text-xl"></i>
            </div>
            <div>
                <div class="text-sm text-500">Usuario Seleccionado</div>
                <div class="text-xl font-bold text-900">{{ userName }}</div>
            </div>
        </div>

        <div class="flex-grow-1 border-1 surface-border border-round-lg overflow-hidden shadow-sm">
            <Splitter class="h-full">
                <SplitterPanel :size="30" class="flex flex-column">
                    <SecurityModulesTable @select="onModuleSelected" />
                </SplitterPanel>
                
                <SplitterPanel :size="70" class="flex flex-column">
                    <Splitter layout="vertical" class="h-full">
                        <SplitterPanel :size="50" class="flex flex-column">
