<template>
  <div class="card p-0 border-round-lg shadow-sm border-1 surface-border overflow-auto mt-2 md-2 ml-2 mr-2">
    ...
    <DataTable :value="modules" stripedRows size="small" selectionMode="single" dataKey="securityPk"
      v-model:selection="selected" @rowSelect="onSelect" scrollable scrollHeight="flex" class="custom-header-table">
      ...
      <Column header="Estado" class="text-center">
        <template #body="slotProps">
          <div class="flex justify-content-center align-items-center gap-2">
            <ToggleSwitch  @click="selectRow(slotProps.data)" v-model="slotProps.data.active" @change="changeModuleState(slotProps.data)" />

            <span class="text-xs font-bold px-2 py-1 border-round-md transition-colors"
...
const changeModuleState = async (module: any) => {
    
    const oldValue = !module.active

    try {

        await updateModuleUser(
            props.userPk!,
            module.securityPk,
            module.active
        )
...
const selectRow = (module: SecurityModule) => {
