...
onMounted(async () => {
    ...
    userName.value = String(userData.userName || '')
...
onMounted(async () => {
    ...
    // Fallback: Si no hay state, intentar leer de sessionStorage
    if (!userData) {
        const stored = sessionStorage.getItem('temp_user');
        if (stored) userData = JSON.parse(stored);
    }
    
    if (!userData) return; // Manejar error
    // ... resto del código
});
