...
onMounted(async () => {
    ...
    if (!userData) {
        console.error("Redirección directa sin estado");
        // Opcional: router.push('/lista-usuarios')
        return;
    }

    // 2. Asignar el nombre (ajusta 'name' según tu interfaz User)
    userName.value = userData.name || 'Usuario desconocido';
    ...
    const response = await fetch(`${import.meta.env.VITE_API_URL}/WebGetSecurityModulesUser`, {
        ...
        body: JSON.stringify({
            // Asegúrate de que el campo sea 'pkid' o 'id' según tu objeto
