        ...
        <section class="login-shell">
            ...
            <aside class="brand-panel">
                ...
                <div class="brand-copy">
                    <span class="brand-eyebrow">Portal del Cliente</span>
                    <h1>Tus tareas,<br />siempre a mano.</h1>
                    <p>Consulta tus tareas asignadas, comenta con tu responsable y crea nuevas solicitudes.</p>
                </div>

                <div class="brand-features">
                    <span><i class="pi pi-check-circle"></i> Tus tareas asignadas</span>
                    <span><i class="pi pi-comments"></i> Comenta y colabora</span>
                    <span><i class="pi pi-shield"></i> Acceso con enlace seguro</span>
                </div>
                ...
                <div class="access-content">
                    <header class="company-header">
                        <div class="client-logo-wrap">
                            <img v-if="company.logo && !logoError" :key="company.logo" :src="company.logo"
                                alt="Logotipo de la empresa" class="client-logo" @error="logoError = true" />
                            <i class="pi pi-building client-logo-fallback"></i>
                        </div>
                        <div>
                            <span class="access-eyebrow">Bienvenido</span>
                            <h2>{{ company.name || 'Portal del Cliente' }}</h2>
                            <p v-if="company.cif">NIF/CIF: {{ company.cif }}</p>
                        </div>
                    </header>

                    <div class="access-intro">
                        <h3>Accede a tus tareas</h3>
                        <p>Indica tu correo y te enviaremos un enlace personal válido 24 horas.</p>
                    </div>

                    <form class="login-form" @submit.prevent="sendLink">
                        <div class="field-group">
                            <label for="portal-email">Correo</label>
                            <IconField>
                                <InputIcon class="pi pi-envelope" />
                                <InputText id="portal-email" v-model="email" autocomplete="email"
                                    placeholder="tu@empresa.com" autofocus fluid />
                            </IconField>
                        </div>

                        <Button type="submit" label="Enviar enlace" icon="pi pi-send" iconPos="right"
                            :loading="sending" :disabled="!email.trim()" class="login-submit" />
                    </form>

                    <Message v-if="loginMsg" :severity="loginOk ? 'success' : 'error'" :closable="false"
                        style="margin-top:1rem">{{ loginMsg }}</Message>

                    <p class="access-help"><i class="pi pi-lock"></i> Tu enlace es personal y caduca en 24 horas.</p>
                </div>

                <footer class="access-footer">
                    <span>KiwiKERP</span><span class="footer-dot"></span><span>Portal del Cliente</span>
                </footer>
            </section>
        </section>
    </main>
