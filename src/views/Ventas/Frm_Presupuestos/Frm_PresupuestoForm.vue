    ...
    <Dialog v-model:visible="visible" modal maximizable :header="quote.pkid ? `${documentLabel} ${quote.code}` : `Nuevo ${documentLabel.toLowerCase()}`"
        ...
        <div class="quote-content flex flex-column gap-4">
            <div v-if="loadingQuote" class="quote-loading-overlay">
                <CorporateLoader :label="'Cargando ' + documentLabel.toLowerCase() + '…'" />
            </div>
            ...
            <div v-if="isOrder" class="order-progress" aria-label="Seguimiento operativo del pedido">
                ...
                <template v-else>
                    ...
                    <div class="order-progress-items">
                        ...
                        <div class="order-progress-item invoice"><i class="pi pi-receipt"></i><div><b>{{ orderProgress.invoiceLines }}</b><span>Pendientes de facturación</span><small>Según política de cada línea</small></div></div>
            ...
            <Panel class="general-data-panel">
                <template #header>
                ...
                <div class="grid p-fluid mb-2">
                    <div class="col-12 md:col-2"><FloatLabel variant="on"><InputText id="quote-code" v-model="quote.code" disabled class="w-full" /><label for="quote-code">Código</label></FloatLabel></div>
                    <div class="col-12 md:col-2"><FloatLabel variant="on"><DatePicker id="quote-date" v-model="createDate" dateFormat="dd/mm/yy" showIcon :disabled="!isEditable || pricing.busy.value" class="w-full" /><label for="quote-date">Fecha</label></FloatLabel><InlineMessage v-if="errors.createDate" severity="error">{{ errors.createDate }}</InlineMessage></div>
                    <div class="col-12 md:col-2"><FloatLabel variant="on"><DatePicker id="quote-validity" v-model="validityDate" :minDate="createDate ?? undefined" dateFormat="dd/mm/yy" showIcon :disabled="!isEditable || pricing.busy.value" class="w-full" /><label for="quote-validity">{{ isOrder ? 'Fecha prevista de entrega' : 'Válido hasta' }}</label></FloatLabel><InlineMessage v-if="errors.validityDate" severity="error">{{ errors.validityDate }}</InlineMessage></div>
                    <div class="col-12 md:col-6"><FloatLabel variant="on"><CustomerLookup :disabled="pricing.busy.value" v-if="isEditable" id="quote-customer" v-model="quote.entityId" :label="customerLabel" @selected="selectCustomer" /><InputText v-else id="quote-customer" :modelValue="customerLabel" disabled class="w-full" /><label for="quote-customer">Cliente</label></FloatLabel><InlineMessage v-if="errors.recipient" severity="error">{{ errors.recipient }}</InlineMessage></div>
                    <div class="col-12 md:col-3"><FloatLabel variant="on"><Select id="quote-rate" v-model="quote.salesTarifaId" :options="rates" optionLabel="description" optionValue="pkid" filter showClear :disabled="!isEditable || pricing.busy.value" class="w-full" /><label for="quote-rate">Tarifa</label></FloatLabel><small class="currency-hint"><i class="pi pi-dollar"></i> Moneda: {{ currencyLabel }}</small><InlineMessage v-if="errors.salesTarifaId" severity="error">{{ errors.salesTarifaId }}</InlineMessage></div>
                    <div class="col-12 md:col-3"><FloatLabel variant="on"><Select id="quote-term" v-model="quote.salesTermId" :options="terms" optionLabel="description" optionValue="pkid" filter showClear :disabled="!isEditable || pricing.busy.value" class="w-full" /><label for="quote-term">Forma de pago</label></FloatLabel><InlineMessage v-if="errors.salesTermId" severity="error">{{ errors.salesTermId }}</InlineMessage></div>
                    <div class="col-12 md:col-3"><FloatLabel variant="on"><Select id="quote-fiscal-position" v-model="quote.retentionId" :options="fiscalPositions" optionLabel="description" optionValue="pkid" filter showClear :disabled="!isEditable || pricing.busy.value" class="w-full" /><label for="quote-fiscal-position">Posición fiscal</label></FloatLabel><InlineMessage v-if="errors.retentionId" severity="error">{{ errors.retentionId }}</InlineMessage></div>
                    <div class="col-12 md:col-3"><FloatLabel variant="on"><InputText id="quote-reference" v-model="quote.reference" :disabled="!isEditable || pricing.busy.value" class="w-full" /><label for="quote-reference">Referencia</label></FloatLabel></div>
                    <div class="col-12 md:col-12"><FloatLabel variant="on"><InputText id="quote-contact" v-model="quote.contact" :disabled="!isEditable || pricing.busy.value" class="w-full" /><label for="quote-contact">Persona de contacto</label></FloatLabel><InlineMessage v-if="errors.recipient" severity="error">{{ errors.recipient }}</InlineMessage></div>
                </div>
            </Panel>

            <div v-if="isEditable"><div class="flex gap-2 align-items-center flex-wrap"><Button label="Aplicar tarifa" icon="pi pi-tags" text :disabled="pricing.busy.value" @click="pricing.request"/><small>La tarifa propone el precio neto. El descuento de línea es adicional.</small></div><Message v-if="pricing.error.value" severity="error">{{pricing.error.value}}</Message></div>
            <Panel class="product-lines-panel">
                ...
                <template #icons><Button label="Añadir línea" icon="pi pi-plus" size="small" :disabled="!canAddLine || !isEditable || pricing.busy.value" @click="addLine" /></template>
                <DataTable ref="quoteLinesTable" :value="quote.lines" size="small" scrollable scrollHeight="300px" stripedRows class="quote-lines" :tableStyle="{ minWidth: isOrder ? '1185px' : '955px' }">
                    <Column header="Producto" style="min-width: 200px"><template #body="{ data }"><ProductLookup :disabled="pricing.busy.value" v-if="isEditable" v-model="data.productId" :label="data.productId ? productLabel(data.productId) : ''" @selected="selectProduct(data, $event)" @cleared="clearProduct(data)" /><InputText v-else :modelValue="productLabel(data.productId)" disabled class="w-full" /></template></Column>
                    <Column header="Descripción" style="min-width: 210px"><template #body="{ data }"><InputText v-model="data.nameProduct" :disabled="!isEditable || pricing.busy.value" class="w-full" /></template></Column>
                    <Column header="Cant." headerClass="compact-number-header" bodyClass="compact-number-cell" style="width: 66px; min-width: 66px; max-width: 66px"><template #body="{ data }"><InputNumber v-model="data.quantity" @update:modelValue="pricing.quantityChanged(data,$event)" locale="de-DE" :min="0" :minFractionDigits="0" :maxFractionDigits="3" :disabled="!isEditable || pricing.busy.value" class="w-full" /></template></Column>
                    <Column v-if="isOrder" style="min-width: 70px"><template #header><span class="line-header delivery"><i class="pi pi-truck"></i> Entregada</span></template><template #body="{ data }"><span class="line-progress-value">{{ formatQuantity(data.quantityDelivered) }}</span></template></Column>
                    <Column v-if="isOrder" style="min-width: 70px"><template #header><span class="line-header invoiced"><i class="pi pi-receipt"></i> Facturada</span></template><template #body="{ data }"><span class="line-progress-value">{{ formatQuantity(data.quantityInvoiced) }}</span></template></Column>
                    <Column v-if="isOrder" style="min-width: 70px"><template #header><span class="line-header cancelled"><i class="pi pi-times-circle"></i> Cancelada</span></template><template #body="{ data }"><span class="line-progress-value">{{ formatQuantity(data.quantityCanceled) }}</span></template></Column>
                    <Column v-if="isOrder" style="min-width: 74px"><template #header><span class="line-header pending"><i class="pi pi-clock"></i> Pendiente</span></template><template #body="{ data }"><span class="line-pending-value">{{ formatQuantity(linePending(data)) }}</span></template></Column>
                    <Column header="UOM" style="min-width: 95px"><template #body="{ data }"><Select v-if="isEditable" v-model="data.uomId" :options="compatibleUoms(data)" optionLabel="description" optionValue="pkid" :disabled="!data.productId || pricing.busy.value" class="w-full" @update:modelValue="selectUom(data, $event)" /><InputText v-else :modelValue="data.uomDescription" disabled class="w-full" /></template></Column>
                    <Column header="Precio" headerClass="compact-number-header" bodyClass="compact-number-cell" style="width: 160px; min-width: 160px"><template #body="{ data }"><div class="line-price"><InputNumber :title="data.pricingSource || 'Precio guardado: origen no registrado'" v-model="data.priceUnit" @input="markManualPrice(data,$event.value)" mode="currency" :currency="currencyCode" locale="de-DE" :useGrouping="true" :min="0" :disabled="!isEditable || pricing.busy.value" class="w-full" /><span v-if="data.pricingPending" class="line-price-origin"><i class="pi pi-spin pi-spinner" aria-hidden="true"/> Calculando…</span><span v-else-if="data.pricingSource || data.pkid" class="line-price-origin" :class="{'line-price-tariff':data.pricingSource?.startsWith('Regla')}" :title="data.pricingSource || 'Origen del precio no registrado'" :aria-label="data.pricingSource || 'Precio guardado'"><i :class="data.pricingSource?.startsWith('Regla')?'pi pi-tags':'pi pi-info-circle'" aria-hidden="true"/> {{data.pricingSource?.startsWith('Regla')?'Tarifa aplicada':data.pricingSource==='Precio manual'?'Precio manual':data.pricingSource?'Precio base':'Precio guardado'}}</span></div></template></Column>
                    <Column header="Dto. %" headerClass="compact-number-header" bodyClass="compact-number-cell" style="width: 64px; min-width: 64px; max-width: 64px"><template #body="{ data }"><InputNumber v-model="data.discountPercent" suffix=" %" :min="0" :max="100" :minFractionDigits="0" :maxFractionDigits="2" :disabled="!isEditable || pricing.busy.value" class="w-full" /></template></Column>
                    <Column header="IVA %" headerClass="compact-number-header" bodyClass="compact-number-cell" style="width: 64px; min-width: 64px; max-width: 64px"><template #body="{ data }"><InputNumber v-model="data.tax" suffix=" %" :min="0" :max="100" :disabled="hasFiscalPosition || !isEditable" class="w-full" /></template></Column>
                    <Column header="Importe" headerClass="amount-column-header" bodyClass="amount-column-cell" style="min-width: 115px"><template #body="{ data }"><span class="line-amount font-semibold">{{ formatCurrency(lineTotal(data)) }}</span></template></Column>
                    <Column style="width: 50px"><template #body="{ index }"><Button icon="pi pi-trash" severity="danger" text rounded :disabled="!isEditable || pricing.busy.value" @click="quote.lines.splice(index, 1)" /></template></Column>
                </DataTable>
                <div class="quote-summary mt-4"><div class="conditions-stack"><div class="conditions-card"><label for="quote-terms">Condiciones generales</label>
                    <Textarea id="quote-terms" v-model="quote.terms" rows="7" :disabled="!isEditable || pricing.busy.value" style="width: 95%" /></div>
                    <div class="conditions-card"><label for="quote-customer-terms">Condiciones particulares del cliente</label>
                    <Textarea id="quote-customer-terms" v-model="quote.customerTerms" rows="7" :disabled="!isEditable || pricing.busy.value" style="width: 95%" /></div></div>
                    
                    
                    <div class="totals-card"><div v-if="totals.discount" class="total-row discount-row"><span><i class="pi pi-tag"></i> Descuento</span><b>- {{ formatCurrency(totals.discount) }}</b></div><div class="total-row"><span><i class="pi pi-wallet"></i> Base imponible</span><b>{{ formatCurrency(totals.net) }}</b></div><div class="total-row"><span><i class="pi pi-percentage"></i> IVA</span><b>{{ formatCurrency(totals.tax) }}</b></div><div v-if="totals.retention" class="total-row retention-row"><span><i class="pi pi-minus-circle"></i> Retención</span><b style="color: #B1D70E">- {{ formatCurrency(totals.retention) }}</b></div><div class="total-divider" style="height: 2px; background: #B1D70E; opacity: .65"></div><div class="total-row grand-total"><span><i class="pi pi-calculator"></i> Total</span><b>{{ formatCurrency(totals.total) }}</b></div></div></div>
            </Panel>

        </div>
        <template #footer><div class="quote-footer"><div class="kiwik-separator quote-footer-separator"></div><div class="quote-footer-actions"><Button label="Cancelar" severity="secondary" text :disabled="loadingQuote || saving" @click="visible = false" /><Button :label="isOrder ? 'Guardar pedido' : 'Guardar borrador'" icon="pi pi-save" :loading="saving" :disabled="loadingQuote || saving || !isEditable || pricing.invalid.value" @click="save" /></div></div></template>
    </Dialog>
...
import { PERM } from '@/services/Frm_Main/permissions'; import Column from 'primevue/column'; import DataTable from 'primevue/datatable'; import DatePicker from 'primevue/datepicker'; import Dialog from 'primevue/dialog'; import FloatLabel from 'primevue/floatlabel'; import InlineMessage from 'primevue/inlinemessage'; import InputNumber from 'primevue/inputnumber'; import InputText from 'primevue/inputtext'; import Message from 'primevue/message'; import Panel from 'primevue/panel'; import CorporateLoader from '@/components/shared/CorporateLoader.vue'; import Select from 'primevue/select'; import Textarea from 'primevue/textarea';
...
const canEditDoc = computed(() => isOrder.value ? securityStore.hasPermission(PERM.ORDER_EDIT) : securityStore.hasPermission(PERM.QUOTE_EDIT));
...
const createQuote = (): SalesQuoteDTO => ({ pkid: null, type: isOrder.value ? 'S' : 'P', code: '', state: isOrder.value ? 'Para Aprobar / To Approved Sale' : 'Para Aprobar / To Approved Estimation', createDate: new Date().toISOString(), validityDate: null, entityId: null, salesTermId: null, salesTarifaId: null, shippingAddressId: null, billingAddressId: null, retentionId: null, reference: '', contact: '', terms: '', customerTerms: '', notes: '', lines: [] });
const quote = ref<SalesQuoteDTO>(createQuote());
const stateEditable = computed(() => isOrder.value ? (!quote.value.pkid || (!quote.value.locked && !['Completado / Completed', 'Cancelado / Canceled', 'Realizado / Done'].includes(quote.value.state))) : (!quote.value.pkid || ['Para Aprobar / To Approved Estimation', 'Enviado / Sended'].includes(quote.value.state)));
const isEditable = computed(() => stateEditable.value && canEditDoc.value);
const pricing = useSalesPricing({lines:()=>quote.value.lines,tarifa:computed({get:()=>quote.value.salesTarifaId,set:(v)=>{quote.value.salesTarifaId=v}}),enabled:()=>visible.value&&!loadingQuote.value&&!saving.value&&isEditable.value,currency:(id)=>rates.value.find(r=>r.pkid===id)?.currencyCode});
...
const clearProduct = (line: SalesQuoteLineDTO) => { pricing.clear(line); line.nameProduct = ''; line.uomId = null; line.uomDescription = ''; line.uomFactor = 1; line.basePriceUnit = 0; line.priceUnit = 0; line.productTax = 0; line.tax = 0; };
...
function markManualPrice(line:SalesQuoteLineDTO,value:string|number|undefined){const price=Number(value);if(loadingQuote.value||saving.value||pricing.busy.value||!Number.isFinite(price)||price===Number(line.priceUnit))return;line.basePriceUnit=price*(line.uomFactor||1);line.pricingSource='Precio manual';}
const selectUom = (line: SalesQuoteLineDTO, uomId: number) => { const uom = uoms.value.find(item => item.pkid === uomId); if (!uom) return; line.uomDescription = uom.description; line.uomFactor = Number(uom.factor ?? 1); line.priceUnit = roundMoney(Number(line.basePriceUnit ?? 0) / (line.uomFactor || 1)); if (pricing.isManaged(line) && line.pricingSource !== "Precio manual") void pricing.apply(line, false); };
const selectProduct = (line: SalesQuoteLineDTO, product: any) => { if (!product) return; const index = products.value.findIndex(item => item.pkid === product.pkid); if (index < 0) products.value.push(product); else products.value[index] = product; line.nameProduct = product.description || ''; line.uomId = product.uomId ?? null; line.uomDescription = product.uomDescription ?? ''; line.uomFactor = Number(product.uomFactor ?? 1); line.basePriceUnit = Number(product.salePrice ?? 0) * (line.uomFactor || 1); line.priceUnit = line.basePriceUnit / (line.uomFactor || 1); line.productTax = Number(product.taxValue ?? 0); line.tax = line.productTax; line.discountPercent=0; applyFiscalPosition(); void pricing.apply(line); };
...
const orderProgress = computed(() => {
    ...
    quote.value.lines.forEach(line => {
        ...
        const pendingDelivery = requiresDelivery ? Math.max(0, quantity - canceled - delivered) : 0;
        const pendingInvoice = Math.max(0, quantity - canceled - invoiced);
        if (pendingDelivery <= 0.0001 && pendingInvoice <= 0.0001) { completedLines++; return; }
        if (pendingDelivery > 0.0001) { deliveryLines++; deliveryQuantity += pendingDelivery; }
        if (pendingInvoice > 0.0001) invoiceLines++;
    });
...
const loadCatalogs = async () => { const { data } = await axios.get(backendUrl(`/WebLoadSalesQuoteCatalog`)); rates.value = data.rates ?? []; terms.value = data.terms ?? []; fiscalPositions.value = [...(data.fiscalPositions ?? [])].sort((left, right) => String(left.description ?? '').localeCompare(String(right.description ?? ''), 'es', { sensitivity: 'base' })); uoms.value = data.uoms ?? []; return data; };
const open = async (pkid: number | null) => { pricing.cancel(); quote.value = createQuote(); createDate.value = new Date(); validityDate.value = null; loadingQuote.value = true; visible.value = true; try { const catalog = await loadCatalogs(); if (pkid) { const { data } = await axios.get(backendUrl(`/${detailEndpoint.value}/${pkid}`)); quote.value = { ...createQuote(), ...data, type: isOrder.value ? 'S' : 'P', lines: (data.lines ?? []).map((line: SalesQuoteLineDTO) => { const uom = uoms.value.find(item => item.pkid === line.uomId); return { ...line, productTax: line.productTax ?? line.tax, uomDescription: line.uomDescription ?? uom?.description ?? '', uomFactor: Number(uom?.factor ?? 1), basePriceUnit: Number(line.priceUnit ?? 0) * Number(uom?.factor ?? 1) }; }) }; if (data.entityId && data.entityName && !customers.value.some(item => item.pkid === data.entityId)) customers.value.push({ pkid: data.entityId, name: data.entityName }); quote.value.lines.forEach(line => { if (line.productId && line.productCode && !products.value.some(item => item.pkid === line.productId)) products.value.push({ pkid: line.productId, code: line.productCode }); }); await nextTick(); createDate.value = toPickerDate(data.createDate); validityDate.value = toPickerDate(data.validityDate); applyFiscalPosition(); } else { quote.value.terms = catalog.defaultTerms ?? ''; quote.value.salesTarifaId=catalog.defaultTarifaId??null; } } catch (error) { console.error(error); toast.add({ severity: 'error', summary: 'Error', detail: `No se pudo cargar el ${documentLabel.value.toLowerCase()} o sus datos auxiliares.`, life: companyStore.companyInfo.toastDuration ?? 3000 }); } finally { loadingQuote.value = false; } };
...
const save = async () => { if (saving.value || pricing.invalid.value || !validate()) return; saving.value = true; try { await axios.post(backendUrl(`/${saveEndpoint.value}`), { ...quote.value, createDate: createDate.value?.toISOString() ?? null, validityDate: validityDate.value?.toISOString() ?? null, userId: authStore.user?.pkid ?? null }); toast.add({ severity: 'success', summary: 'Guardado correctamente', life: companyStore.companyInfo.toastDuration ?? 3000 }); visible.value = false; emit('saved'); } catch (error: any) { toast.add({ severity: 'error', summary: 'Error', detail: error.response?.data ?? `No se pudo guardar el ${documentLabel.value.toLowerCase()}.`, life: companyStore.companyInfo.toastDuration ?? 3000 }); } finally { saving.value = false; } };
defineExpose({ open });
</script>

<style scoped>
.line-price{display:flex;flex-direction:column;gap:.35rem;min-width:0}.line-price-origin{display:inline-flex;align-items:center;justify-content:flex-end;gap:.3rem;font-size:.78rem;line-height:1.3;color:#707b8c;white-space:nowrap}.line-price-tariff{color:#648506;font-weight:600}.line-price-origin i{font-size:.8rem}
.quote-lines :deep(.p-inputnumber), .quote-lines :deep(.p-select) { min-width: 100%; }
.quote-lines :deep(.p-datatable-thead > tr > th), .quote-lines :deep(.p-datatable-tbody > tr > td) { padding-inline: .55rem; }
.quote-lines :deep(.p-datatable-tbody > tr > td) { vertical-align: top; }
.quote-lines :deep(.p-inputtext), .quote-lines :deep(.p-select-label) { padding-inline: .55rem; }
.quote-lines :deep(.compact-number-header), .quote-lines :deep(.compact-number-cell) { padding-inline: .25rem; text-align: center; }
.quote-lines :deep(.compact-number-header .p-datatable-column-header-content) { justify-content: center; }
.quote-lines :deep(.compact-number-cell .p-inputnumber) { width: 100%; min-width: 0; max-width: 100%; }
.quote-lines :deep(.compact-number-cell .p-inputnumber-input) { box-sizing: border-box; width: 100%; min-width: 0; max-width: 100%; padding-inline: .3rem; text-align: right; }
.quote-lines :deep(.amount-column-header .p-datatable-column-header-content) { justify-content: flex-end; }
.quote-lines :deep(.amount-column-cell) { text-align: right; }
.line-amount { display: block; width: 100%; text-align: right; white-space: nowrap; font-variant-numeric: tabular-nums; }
...
.general-data-panel :deep(.p-panel-content) { overflow: visible; }
.general-data-panel :deep(.p-floatlabel) { display: block; padding-top: .35rem; overflow: visible; }
.general-data-panel :deep(.p-datepicker) { width: 100%; }
.general-data-panel :deep(.p-floatlabel > label) { z-index: 2; max-width: calc(100% - 1rem); padding: 0 .3rem; line-height: 1.25; white-space: nowrap; background: var(--surface-0); }
...
.quote-summary { box-sizing:border-box; width:100%; max-width:100%; min-width:0; overflow:hidden; display:grid; grid-template-columns:minmax(180px,calc(17% - .5rem)) minmax(0,calc(83% - .5rem)); align-items:start; gap:1rem; }.conditions-stack { order:2; display:flex; flex-direction:column; gap:1rem; width:100%; min-width:0; }.conditions-card,.totals-card { box-sizing:border-box; width:100%; min-width:0; max-width:100%; overflow:hidden; }.totals-card { order:1; }.conditions-card { padding:1rem 1.25rem; border:1px solid var(--surface-200); border-radius:12px; background:var(--surface-0); box-shadow:0 5px 14px rgba(0,0,0,.05); }.conditions-card label { display:block; margin-bottom:.55rem; color:var(--text-color-secondary); font-size:.9rem; font-weight:600; }.quote-footer { box-sizing:border-box; width:100%; }.quote-footer-separator { width:100%; min-height:1px; margin:0 0 .75rem; border-top:1px solid var(--surface-300); }.quote-footer-actions { display:flex; justify-content:flex-end; gap:.5rem; }
...
.line-progress-value { box-sizing:border-box; display:inline-flex; width:100%; min-width:0; justify-content:flex-end; padding:.48rem .45rem; border-radius:6px; background:#f8fafc; color:#6b7280; border:1px solid #e5e7eb; font-weight:600; font-variant-numeric:tabular-nums; }.line-pending-value { box-sizing:border-box; display:inline-flex; width:100%; min-width:0; justify-content:flex-end; padding:.48rem .45rem; border-radius:6px; background:#eef6da; color:#5f810e; border:1px solid #dceab9; font-weight:700; font-variant-numeric:tabular-nums; }
