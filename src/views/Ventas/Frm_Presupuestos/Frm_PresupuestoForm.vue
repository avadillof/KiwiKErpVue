        ...
        <div class="quote-content flex flex-column gap-4">
            ...
            <Panel class="general-data-panel">
                ...
                <div class="grid p-fluid mb-2">
                    ...
                    <div class="col-12 md:col-2"><FloatLabel variant="on"><DatePicker id="quote-validity" v-model="validityDate" :minDate="createDate ?? undefined" dateFormat="dd/mm/yy" showIcon :disabled="!isEditable" class="w-full" /><label for="quote-validity">{{ isOrder ? 'Fecha prevista de entrega' : 'Válido hasta' }}</label></FloatLabel><InlineMessage v-if="errors.validityDate" severity="error">{{ errors.validityDate }}</InlineMessage></div>
                    <div class="col-12 md:col-6"><FloatLabel variant="on"><CustomerLookup v-if="isEditable" id="quote-customer" v-model="quote.entityId" :label="customerLabel" @selected="selectCustomer" /><InputText v-else id="quote-customer" :modelValue="customerLabel" disabled class="w-full" /><label for="quote-customer">Cliente</label></FloatLabel><InlineMessage v-if="errors.recipient" severity="error">{{ errors.recipient }}</InlineMessage></div>
            ...
            <Panel class="product-lines-panel">
                ...
                <DataTable ref="quoteLinesTable" :value="quote.lines" size="small" scrollable scrollHeight="300px" stripedRows class="quote-lines" :tableStyle="{ minWidth: isOrder ? '1185px' : '955px' }">
                    <Column header="Producto" style="min-width: 200px"><template #body="{ data }"><ProductLookup v-if="isEditable" v-model="data.productId" :label="data.productId ? productLabel(data.productId) : ''" @selected="selectProduct(data, $event)" @cleared="clearProduct(data)" /><InputText v-else :modelValue="productLabel(data.productId)" disabled class="w-full" /></template></Column>
                    <Column header="Descripción" style="min-width: 210px"><template #body="{ data }"><InputText v-model="data.nameProduct" :disabled="!isEditable" class="w-full" /></template></Column>
                    <Column header="Cant." headerClass="compact-number-header" bodyClass="compact-number-cell" style="width: 66px; min-width: 66px; max-width: 66px"><template #body="{ data }"><InputNumber v-model="data.quantity" locale="de-DE" :min="0" :minFractionDigits="0" :maxFractionDigits="3" :disabled="!isEditable" class="w-full" /></template></Column>
                    ...
                    <Column header="UOM" style="min-width: 95px"><template #body="{ data }"><Select v-if="isEditable" v-model="data.uomId" :options="compatibleUoms(data)" optionLabel="description" optionValue="pkid" :disabled="!data.productId" class="w-full" @update:modelValue="selectUom(data, $event)" /><InputText v-else :modelValue="data.uomDescription" disabled class="w-full" /></template></Column>
                    <Column header="Precio" headerClass="compact-number-header" bodyClass="compact-number-cell" style="width: 86px; min-width: 86px; max-width: 86px"><template #body="{ data }"><InputNumber v-model="data.priceUnit" mode="currency" :currency="currencyCode" locale="de-DE" :min="0" :disabled="!isEditable" class="w-full" /></template></Column>
                    <Column header="Dto. %" headerClass="compact-number-header" bodyClass="compact-number-cell" style="width: 64px; min-width: 64px; max-width: 64px"><template #body="{ data }"><InputNumber v-model="data.discountPercent" suffix=" %" :min="0" :max="100" :minFractionDigits="0" :maxFractionDigits="2" :disabled="!isEditable" class="w-full" /></template></Column>
                    ...
                    <Column style="width: 50px"><template #body="{ index }"><Button icon="pi pi-trash" severity="danger" text rounded :disabled="!isEditable" @click="quote.lines.splice(index, 1)" /></template></Column>
                </DataTable>
                <div class="quote-summary mt-4"><div class="conditions-stack"><div class="conditions-card"><label for="quote-terms">Condiciones generales</label>
                    <Textarea id="quote-terms" v-model="quote.terms" rows="7" :disabled="!isEditable" style="width: 95%" /></div>
                    <div class="conditions-card"><label for="quote-customer-terms">Condiciones particulares del cliente</label>
                    <Textarea id="quote-customer-terms" v-model="quote.customerTerms" rows="7" :disabled="!isEditable" style="width: 95%" /></div></div>
