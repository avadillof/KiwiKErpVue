  ...
  <div class="sales-page">
    <header class="page-header">
      <div class="page-heading">
        ...
        <div>
          <span class="breadcrumb">Ventas / Operaciones</span>
          <h1>Presupuestos</h1>
          <p>Ofertas, borradores y presupuestos enviados a clientes.</p>
        </div>
      </div>
    ...
    <div class="card list-card">
      <Toolbar class="list-toolbar">
        <template #start
          ><div class="workspace-heading">
            ...
            ><small>Consulta, filtra y gestiona las ofertas comerciales.</small>
          </div></template
        >
        <template #end
          ><div class="module-actions">
            <SalesAutomationActions
              module="quotes"
              @executed="
                selectedQuote = null;
                refreshTable();
      ...
      <GenericDataTable
        ...
        v-model:selection="selectedQuote"
        endpoint="WebGetSalesQuotes"
        :params="{ type: 'P', state: selectedState || undefined }"
        :showPaginator="true"
        :filterable="true"
        :showActions="true"
        @row-select="({ data }) => (selectedQuote = data)"
        @search="clearDateFilters"
      >
        <template #panelOptions>
          <div class="quick-filters">
            <Select
              ...
              class="state-quick-filter"
            >
              <template #value="{ value, placeholder }"
                ><Tag
                  v-if="value"
                  :value="
                    quoteStates.find((item) => item.value === value)?.label ??
                    value
                  "
              ...
              <template #option="{ option }"
                ><Tag
                  :value="option.label"
                  :severity="stateSeverity(option.value)"
                  rounded
              /></template>
            </Select>
            <Select
              v-model="selectedExpirationFilter"
              :options="expirationQuickFilters"
              optionLabel="label"
              optionValue="value"
              showClear
              placeholder="Vencimiento: Todos"
              class="expiration-quick-filter"
            >
              <template #value="{ value, placeholder }"
                ><span v-if="value"
                  ><i class="pi pi-calendar-clock mr-2 text-primary"></i
                  >{{
                    expirationQuickFilters.find((item) => item.value === value)
                      ?.label
                  }}</span
                ><span v-else>{{ placeholder }}</span></template
              >
        ...
        <Column
          field="code"
          sortField="salesOrdersDsCode"
          header="Código"
          sortable
          style="width: 10%"
          bodyClass="nowrap-cell"
          ><template #body="{ data }"
            ><span class="code-with-attachments"
              ><span
                :class="{
                  'draft-code':
                    isDraftEstimation(data.state) ||
                    data.code?.startsWith('DRAFT_'),
                }"
                >{{ data.code }}</span
              ><Tag
                v-if="Number(data.attachmentCount || 0) > 0"
                :value="String(data.attachmentCount)"
                icon="pi pi-paperclip"
                severity="info"
                rounded
                title="Documentos adjuntos" /><i
                v-if="data.hasNotes"
                class="pi pi-comment notes-indicator"
        ...
        <Column
          field="contact"
          sortField="salesOrdersDsContact"
          header="Contacto"
          sortable
          style="width: 10%"
        />
          ...
          <template #header
            ...
            >
              ...
              ><Button
                ...
                rounded
                size="small"
                @click.stop="creationDatePanel?.toggle($event)"
              /></div
          ></template>
          <template #body="{ data }">{{
        ...
        <Column
          ...
          ><template #body="{ data }"
            ><span
              class="creator-cell"
              :title="data.creatorName || data.creatorCode"
              ><Avatar shape="circle" class="creator-avatar"
                ><img
                  v-if="data.creatorId && !creatorPhotoErrors[data.creatorId]"
                  :src="getCreatorPhotoUrl(data.creatorId)"
                  :alt="data.creatorName || data.creatorCode"
                  @error="handleCreatorPhotoError(data.creatorId)"
                /><span v-else>{{
                  creatorInitials(data.creatorName || data.creatorCode)
                }}</span></Avatar
              ><span>{{
                data.creatorName || data.creatorCode || "-"
              }}</span></span
            ></template
          ></Column
        >
        <Column
          field="validityDate"
          sortField="salesOrdersDtValidity"
          sortable
          style="width: 10%"
        >
          <template #header
            ><div
              class="flex align-items-center justify-content-between w-full"
            >
              <span><b>Validez</b></span
              ><Button
                :severity="validityDateRange ? 'danger' : 'secondary'"
                icon="pi pi-filter"
                rounded
                size="small"
                @click.stop="validityDatePanel?.toggle($event)"
              /></div
          ></template>
          <template #body="{ data }">{{
        ...
        <Column
          field="sendDate"
          sortField="salesOrdersDtDatesend"
          header="Fecha envío"
          sortable
          style="width: 10%"
        >
          <template #body="{ data }">{{
            data.sendDate ? formatDateTime(data.sendDate) : "-"
          }}</template>
        </Column>
        <Column
          field="totalTotal"
          sortField="salesOrdersDbTotalTotal"
          header="Total"
          sortable
        ...
        >
          <template #body="{ data }">{{
            formatCurrency(data.totalTotal, data.currencyCode)
          }}</template>
        </Column>
        <Column
          field="state"
          sortField="salesOrdersDsState"
          header="Estado"
          sortable
          style="width: 10%"
        >
        ...
        <Column header="Acciones" style="width: 4%">
          <template #body="{ data }"
            ><Button
              ...
              text
              rounded
              @click="openQuoteMenu($event, data)"
          /></template>
        </Column>
      </GenericDataTable>
    ...
    <section class="quote-stats-card">
      <div class="stats-header">
        <div>
          <h6>
            <i class="pi pi-chart-line"></i> Resumen comercial de presupuestos
          </h6>
          <span>Indicadores del año seleccionado</span>
        </div>
        <div class="flex align-items-center gap-2">
          <Select
            v-model="selectedStatisticsYear"
            :options="statisticsYears"
            class="statistics-year"
          /><Button
            :icon="
              statisticsExpanded ? 'pi pi-chevron-up' : 'pi pi-chevron-down'
            "
            text
            rounded
      ...
      <div v-if="statisticsExpanded" class="stats-body">
        <div v-if="loadingStatistics" class="stats-loading">
          <i class="pi pi-spin pi-spinner"></i> Cargando indicadores...
        </div>
        <template v-else>
          <div class="kpi-grid">
            <div class="kpi-card">
              <span class="kpi-icon total"
                ><i class="pi pi-file-edit"></i
              ></span>
              <div>
                <small>Presupuestos</small
                ><strong>{{ statistics.totalCount }}</strong
                ><em>{{ formatCurrency(statistics.totalAmount) }}</em>
              </div>
            </div>
            <div class="kpi-card">
              <span class="kpi-icon approved"
                ><i class="pi pi-check-circle"></i
              ></span>
              <div>
                <small>Aprobados</small
                ><strong>{{ statistics.approvedCount }}</strong
                ><em>{{ formatCurrency(statistics.approvedAmount) }}</em>
              </div>
            </div>
            <div class="kpi-card">
              <span class="kpi-icon pending"><i class="pi pi-clock"></i></span>
              <div>
                <small>Pendientes</small
                ><strong>{{ statistics.pendingCount }}</strong
                ><em>{{ formatCurrency(statistics.pendingAmount) }}</em>
              </div>
            </div>
            <div class="kpi-card">
              <span class="kpi-icon rate"
                ><i class="pi pi-percentage"></i
              ></span>
              <div>
                <small>Tasa de aceptación</small
                ><strong>{{ statistics.acceptanceRate }} %</strong
                ><em>Sobre presupuestos creados</em>
              </div>
            </div>
            <div class="kpi-card">
              <span
                class="kpi-icon"
                :class="
                  statistics.yearOverYearRate === null ||
                  statistics.yearOverYearRate >= 0
                    ? 'comparison-up'
                    : 'comparison-down'
                "
                ><i
              ...
              ></span>
              <div>
                <small>Comparativa {{ selectedStatisticsYear - 1 }}</small
                ><strong>{{
                  formatCurrency(statistics.previousYearAmount)
                }}</strong
                ><em
                  :class="{
                    'negative-comparison':
                  ...
                  >{{
                    statistics.yearOverYearRate === null
                      ...
                      : `${statistics.yearOverYearRate >= 0 ? "+" : ""}${statistics.yearOverYearRate} % vs. año anterior`
                  }}</em
                >
              </div>
            </div>
          ...
          <div class="top-clients-grid">
            <div class="top-clients-card">
              <h6>
                <i class="pi pi-trophy"></i> Top 5 clientes - Presupuestos
                aprobados
              </h6>
              <div
                v-if="!statistics.topApprovedCustomers.length"
                class="empty-ranking"
              >
                No hay presupuestos aprobados en {{ selectedStatisticsYear }}.
              </div>
              <div
                v-else
                v-for="(customer, index) in statistics.topApprovedCustomers"
                :key="`approved-${customer.code}-${index}`"
                class="ranking-row"
              >
            ...
            <div class="top-clients-card cancelled">
              <h6>
                <i class="pi pi-ban"></i> Top 5 clientes - Presupuestos
                cancelados
              </h6>
              <div
                v-if="!statistics.topCancelledCustomers.length"
                class="empty-ranking"
              >
                No hay presupuestos cancelados en {{ selectedStatisticsYear }}.
              </div>
              <div
                v-else
                v-for="(customer, index) in statistics.topCancelledCustomers"
                :key="`cancelled-${customer.code}-${index}`"
                class="ranking-row"
              >
                <span class="ranking-position">{{ index + 1 }}</span>
                <div class="ranking-customer">
    ...
    <Dialog
      ...
      modal
      header="Enviar presupuesto por correo"
      :style="{ width: '34rem' }"
      :closable="!sendingQuote"
    >
      <div class="send-dialog-intro">
        <i class="pi pi-send"></i
        ><span
          >Selecciona el contacto que recibirá el presupuesto
          <b>{{ selectedQuote?.code }}</b
          >.</span
        >
      </div>
      <div v-if="loadingRecipients" class="flex justify-content-center py-5">
        <i class="pi pi-spin pi-spinner text-primary text-2xl"></i>
      </div>
      <Message
        v-else-if="!emailRecipients.length"
        severity="warn"
        :closable="false"
        >Este cliente no tiene contactos activos con una dirección de correo
        válida.</Message
      >
      <div v-else class="recipient-list">
        <label
          v-for="contact in emailRecipients"
          :key="contact.pkid"
          class="recipient-item"
          :class="{ selected: selectedRecipientId === contact.pkid }"
        >
          <RadioButton
            ...
            :value="contact.pkid"
          />
          <span class="recipient-data"
            ><b>{{ contact.name }}</b
            ><small>{{ contact.email }}</small></span
          >
          <Tag
    ...
    <AttachmentsDialog
      ...
      moduleFolder="ATTACHEMENTS_SALESPRESU_DOCUMENTS"
      :title="`Documentos indexados al presupuesto ${selectedQuote?.code ?? ''}`"
    ...
    /></OverlayPanel>
    <OverlayPanel ref="validityDatePanel" appendTo="body"
      ><DateRangePicker v-model="validityDateRange"
    /></OverlayPanel>
  </div>
</template>
...
const emailRecipients = ref<
  Array<{ pkid: number; name: string; email: string; defaultContact: boolean }>
>([]);
...
const statisticsYears = Array.from(
  { length: 5 },
...
const statistics = ref({
  ...
  previousYearCount: 0,
  yearOverYearRate: null as number | null,
  topApprovedCustomers: [] as Array<{
    name: string;
    code: string;
    count: number;
    amount: number;
  }>,
  topCancelledCustomers: [] as Array<{
    name: string;
    code: string;
    count: number;
    amount: number;
  ...
  months: [] as string[],
  monthlyTotal: [] as number[],
  monthlyApproved: [] as number[],
});
const quoteStates = [
  {
    label: "Pendientes de enviar",
    value: "Para Aprobar / To Approved Estimation",
  },
...
const creatorInitials = (creator?: string) =>
  (creator || "?")
    .trim()
    .split(/\s+/)
    .filter(Boolean)
...
const stateLabels: Record<string, string> = {
  "Para Aprobar / To Approved Estimation": "Pendientes",
  "Enviado / Sended": "Enviados",
  "Aprobado / Approved": "Aprobados",
  "Cancelado / Canceled": "Cancelados",
};
const stateChartData = computed(() => ({
  labels: Object.keys(statistics.value.stateCounts).map(
    (state) => stateLabels[state] ?? state,
  ),
  datasets: [
    {
      data: Object.values(statistics.value.stateCounts),
      backgroundColor: ["#f4c542", "#3b82f6", "#84cc16", "#ef4444"],
      borderColor: "#fff",
      borderWidth: 3,
    },
  ],
...
const monthlyChartData = computed(() => ({
  labels: statistics.value.months,
  datasets: [
    {
      label: "Importe presupuestado",
      data: statistics.value.monthlyTotal,
      backgroundColor: "#d9ef9c",
      borderColor: "#b1d70e",
      borderWidth: 1,
      borderRadius: 4,
    },
    {
      ...
      data: statistics.value.monthlyApproved,
...
const monthlyChartOptions = {
  ...
  plugins: {
    legend: { position: "top", labels: { boxWidth: 11, usePointStyle: true } },
  },
  scales: {
...
const isPendingQuote = (quote: any) =>
  isDraftQuote(quote) || isSentQuote(quote);
const isCancelledQuote = (quote: any) =>
  quote?.state === "Cancelado / Canceled";
...
const noteRequest = {
  table: "SALES_ORDERS",
  pkField: "SALES_ORDERS_PK_ID",
  field: "SALES_ORDERS_DS_NOTES",
  id: -1,
};
...
const loadQuoteStatistics = async () => {
  ...
  try {
    ...
    toast.add({
      severity: "error",
      summary: "No se pudieron cargar las estadísticas",
      life: 3000,
    });
  } finally {
...
const openSendDialog = async () => {
  ...
  try {
    ...
    selectedRecipientId.value =
      emailRecipients.value.find((contact) => contact.defaultContact)?.pkid ??
      emailRecipients.value[0]?.pkid ??
      null;
  } catch (error: any) {
    showSendDialog.value = false;
    toast.add({
      severity: "error",
      summary: "No se pueden cargar los contactos",
      detail: error.response?.data ?? "Inténtalo de nuevo.",
      life: 4000,
    });
  } finally {
...
const sendQuoteByEmail = async () => {
  ...
  try {
    ...
    toast.add({
      ...
      detail: "El PDF se ha enviado al contacto seleccionado.",
      life: 4000,
    });
    ...
    toast.add({
      ...
      detail: error.response?.data ?? "Revisa la configuración de correo.",
      life: 5000,
    });
  } finally {
...
const requestQuoteAction = (action: "accept" | "cancel" | "reopen") => {
  ...
  confirm.require({
    ...
    message: accepting
      ? "Se aprobará el presupuesto, quedará bloqueado y se creará un pedido de venta en borrador."
      : reopening
        ? "El presupuesto volverá a estar pendiente de aprobación y podrá modificarse."
        : "El presupuesto quedará cancelado y bloqueado. Esta acción no se puede deshacer.",
    icon: accepting
      ? "pi pi-check-circle"
      : reopening
        ? "pi pi-undo"
    ...
    accept: async () => {
      try {
        ...
        const detail = accepting
          ...
          : reopening
            ? "Presupuesto reabierto para su revisión."
            : "Presupuesto cancelado y bloqueado.";
        toast.add({
          severity: "success",
          summary: accepting
...
const applyDateFilters = () => {
  ...
  if (creationDateRange.value?.[0] && creationDateRange.value?.[1]) {
    filters.push(
      `[dateCreate],[${creationDateRange.value[0].toISOString()}],[${creationDateRange.value[1].toISOString()}]`,
    );
  }
  const expirationRange = quickExpirationRange();
  if (expirationRange) {
    filters.push(
      `[dateValidity],[${expirationRange.start.toISOString()}],[${expirationRange.end.toISOString()}]`,
    );
  } else if (validityDateRange.value?.[0] && validityDateRange.value?.[1]) {
    filters.push(
      `[dateValidity],[${validityDateRange.value[0].toISOString()}],[${validityDateRange.value[1].toISOString()}]`,
    );
  }
...
const quickExpirationRange = (): { start: Date; end: Date } | null => {
  ...
  if (selectedExpirationFilter.value === "today")
    return { start: today, end: tomorrow };
  if (selectedExpirationFilter.value === "next7days") {
    const end = new Date(today);
    end.setDate(end.getDate() + 8);
    return { start: today, end };
  }
  if (selectedExpirationFilter.value === "expired")
...
const formatDateTime = (value?: string | null) => {
  ...
  const date = parseServerDate(value);
  return date
    ? `${formatDateParts(date)} ${pad(date.getHours())}:${pad(date.getMinutes())}:${pad(date.getSeconds())}`
    : value;
};
...
const parseServerDate = (value: string): Date | null => {
  ...
  const match = value
    .replace(/\u00a0/g, " ")
    .match(
      /^([a-záéíóú]+)\.?\s+(\d{1,2}),\s+(\d{4})\s+(\d{1,2}):(\d{2}):(\d{2})\s+([ap])\.\s*m\.$/i,
    );
...
const isDraftEstimation = (state?: string) =>
  state === "Para Aprobar / To Approved Estimation";
const stateSeverity = (state = "") => {
  ...
  if (
    ...
    state.includes("Aceptado") ||
    state.includes("Facturado")
  )
    return "success";
  if (state.includes("Realizado")) return "contrast";
  if (state.includes("A Facturar") || state.includes("Facturando"))
    return "info";
  return "warn";
};
</script>
...
.page-icon {
  ...
  border-radius: 13px;
  color: #fff;
  background: linear-gradient(135deg, #8d78dc, #6250ad);
  box-shadow: 0 7px 15px rgba(98, 80, 173, 0.22);
}
.page-icon i {
  font-size: 1.3rem;
}
...
.list-card {
  flex: 0 0 auto;
  height: clamp(500px, calc(100dvh - 270px), 720px);
  min-height: 0;
  display: flex;
  flex-direction: column;
  overflow: hidden;
...
.list-toolbar :deep(.new-document) {
  border-color: var(--kiwi);
  background: var(--kiwi);
  color: #253000;
}
.list-toolbar :deep(.new-document:hover) {
  border-color: #8bad09;
  background: #8bad09;
  color: #253000;
}
...
.code-with-attachments :deep(.p-tag) {
  flex: 0 0 auto;
  font-size: 0.72rem;
...
.kpi-card {
  ...
  border-radius: 10px;
  background: linear-gradient(135deg, #fff, #fbfdf5);
}
.kpi-icon {
  width: 38px;
  height: 38px;
...
.kpi-card em.negative-comparison {
  color: #d13d3d;
}
.charts-grid {
  display: grid;
...
.top-clients-card h6 i {
  color: #76ad16;
  margin-right: 0.4rem;
}
.top-clients-card.cancelled h6 {
  color: #8f3d3d;
}
.top-clients-card.cancelled h6 i {
  color: #df4b4b;
}
.ranking-row {
  display: flex;
...
.list-card :deep(.p-datatable-thead > tr > th),
.list-card :deep(.p-datatable-tbody > tr > td) {
  font-size: 0.875rem;
}
.stats-header span {
  font-size: 0.88rem;
