...
const open=async(item:any)=>{if(!item?.pkid)return;visible.value=true;loading.value=true;delivery.value=item;try{const{data}=await axios.get(backendUrl(`/WebGetSalesDelivery/${item.pkid}`));delivery.value=data;deliveryDate.value=parseDate(data.deliveryDate)||new Date();receiverName.value=data.receiverName||'';signatureExists.value=Boolean(data.signatureExists);}catch(e:any){visible.value=false;toast.add({severity:'error',summary:'No se pudo abrir la recogida',detail:e.response?.data||'Inténtalo de nuevo.',life:4000});}finally{loading.value=false;}};
const save=async()=>{if(!deliveryDate.value){toast.add({severity:'warn',summary:'Fecha obligatoria',life:3000});return;}if(!receiverName.value.trim()){toast.add({severity:'warn',summary:'Transportista obligatorio',detail:'Indica quién o qué empresa retira la mercancía.',life:3500});return;}saving.value=true;try{const{data}=await axios.put(backendUrl(`/WebRegisterSalesDeliveryReceipt/${delivery.value.pkid}`),{deliveryDate:deliveryDate.value.toISOString(),receiverName:receiverName.value.trim(),signatureDataUrl:null});visible.value=false;emit('saved',data);toast.add({severity:'success',summary:'Recogida registrada',detail:'Se han guardado la fecha y el transportista que retiró la mercancía.',life:4000});}catch(e:any){toast.add({severity:'error',summary:'No se pudo registrar la recogida',detail:e.response?.data||'Revisa los datos indicados.',life:5000});}finally{saving.value=false;}};
defineExpose({open});
</script>
