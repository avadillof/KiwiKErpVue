<template>
  <Dialog v-model:visible="visible" modal maximizable :closable="!busy && !retryRequest" :style="{width:'min(1100px,96vw)',height:'90vh',maxHeight:'94vh',display:'flex',flexDirection:'column',overflow:'hidden'}" :pt="{root:{class:'kiwik-dialog invoice-payments-dialog'},header:{class:'kiwik-dialog-header'},content:{class:'kiwik-dialog-content'},footer:{class:'kiwik-dialog-footer'}}">
    ...
    <template v-else-if="data">
      ...
      <h3>Vencimientos</h3>
      <InvoiceDuesTable :rows="data.dues||[]" :currency="invoice?.currencyCode||'EUR'"/>
      ...
      <DataTable :value="data.history" dataKey="id" scrollable scrollHeight="260px" size="small">
        ...
        <Column header="Plazos liquidados"><template #body="{data:p}"><div v-for="a in (data.allocations||[]).filter((a:any)=>a.paymentId===p.id)" :key="a.id">Plazo {{data.dues?.find((d:any)=>d.id===a.dueId)?.position}}: {{money(a.amount)}}</div></template></Column>
      ...
      <fieldset v-if="data.canReceivePayment || retryRequest" :disabled="busy||!!retryRequest">
        ...
        <label><input type="checkbox" v-model="manualAllocation" @change="allocations={}"> Elegir plazos e importes</label>
        <p v-if="!manualAllocation">Asignación automática al vencimiento pendiente más antiguo; el sobrante pasa al siguiente.</p>
        <div v-else class="form-grid">
          <label v-for="d in (data.dues||[]).filter((d:any)=>Number(d.pendingAmount)>0)" :key="d.id">Plazo {{d.position}} · {{formatDate(d.dueDate)}} · Pendiente {{money(d.pendingAmount)}}<InputNumber v-model="allocations[d.id]" :min="0" :max="Number(d.pendingAmount)" :maxFractionDigits="2" @update:modelValue="sumAllocation"/></label>
        </div>
        <Button label="Cobrar saldo completo" text size="small" @click="manualAllocation=false;allocations={};form.amount=Number(data.pendingAmount)"/>
        <div class="form-grid"><label>Fecha<DatePicker v-model="form.date" dateFormat="dd/mm/yy" showIcon :maxDate="today"/></label><label>Importe<InputNumber v-model="form.amount" mode="currency"  :currency="invoice?.currencyCode||'EUR'" :disabled="manualAllocation" locale="es-ES" :min="0.01" :max="Number(data.pendingAmount)" :minFractionDigits="2" :maxFractionDigits="2"/></label><label>Medio de cobro<Select v-model="form.method" :options="methods" optionLabel="label" optionValue="value" placeholder="Selecciona un medio"/></label><label>Referencia<InputText v-model="form.reference" maxlength="200"/></label><label class="wide">Observaciones<Textarea v-model="form.notes" rows="2" maxlength="1000"/></label></div>
...
const manualAllocation=ref(false),allocations=ref<Record<string,number|null>>({});
const allocationRows=()=>Object.entries(allocations.value).filter(([,amount])=>Number(amount)>0).map(([dueId,amount])=>({dueId:Number(dueId),amount:Number(amount)}));
const sumAllocation=()=>{form.value.amount=Math.round(allocationRows().reduce((sum,a)=>sum+a.amount,0)*100)/100;};
const money=(v:any)=>new Intl.NumberFormat('es-ES',{style:'currency',currency:invoice.value?.currencyCode||'EUR'}).format(Number(v)||0);
...
async function register(){
  ...
  const request=retryRequest.value||{allocations:manualAllocation.value?allocationRows():null,operationKey:crypto.randomUUID(),paymentDate:dateString(form.value.date),amount:form.value.amount,method:form.value.method,reference:form.value.reference,notes:form.value.notes};
  retryRequest.value=request;busy.value=true;error.value='';
  try{data.value=(await axios.post(api('WebRegisterSalesInvoicePayment'),{...request,userCode:connectedUserCode()},{timeout:20000})).data;retryRequest.value=null;form.value.amount=null;allocations.value={};manualAllocation.value=false;form.value.reference='';form.value.notes='';emit('saved')}
...
<style scoped>
/* Override the corporate overflow/height rules only for this teleported dialog. */
:global(.invoice-payments-dialog.kiwik-dialog .p-dialog-content) {
...
:global(.invoice-payments-dialog.kiwik-dialog .p-dialog-header),
:global(.invoice-payments-dialog.kiwik-dialog .p-dialog-footer) {
