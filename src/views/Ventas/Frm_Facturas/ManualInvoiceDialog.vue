  ...
  <Dialog v-model:visible="visible" modal maximizable :closable="!locked" :closeOnEscape="!locked" :style="{width:'min(1500px,98vw)',height:'92vh'}" :contentStyle="{overflow:'auto'}" class="kiwik-dialog manual-invoice-dialog">
    ...
    <div class="manual-content">
      ...
      <DataTable ref="linesTable" :value="form.lines" dataKey="key" stripedRows size="small" scrollable scrollHeight="340px" class="manual-lines" :tableStyle="{minWidth:'1120px',tableLayout:'fixed'}">
        <Column header="Artículo / servicio" style="width:200px"><template #body="{data}"><ProductLookup v-model="data.productId" :label="data.productLabel" :disabled="locked" @selected="selectProduct(data,$event)" @cleared="clearProduct(data)"/></template></Column>
        <Column header="Descripción" style="min-width:350px"><template #body="{data}"><InputText v-model="data.description" maxlength="165" :disabled="locked" fluid/></template></Column>
        <Column header="Cantidad" style="width:100px"><template #body="{data}"><InputNumber locale="es-ES" :useGrouping="true" v-model="data.quantity" :min="0.001" :max="1000000000" :maxFractionDigits="3" :disabled="locked" fluid/></template></Column>
        <Column header="Precio" style="width:115px"><template #body="{data}"><InputNumber locale="es-ES" :useGrouping="true" v-model="data.priceUnit" :min="0" :max="1000000000" :minFractionDigits="2" :maxFractionDigits="4" :disabled="locked" fluid/></template></Column>
        <Column header="Dto. %" style="width:80px"><template #body="{data}"><InputNumber locale="es-ES" :useGrouping="true" v-model="data.discount" :min="0" :max="100" :maxFractionDigits="2" :disabled="locked" fluid/></template></Column>
      ...
      <div class="manual-terms"><label><span>Condiciones generales</span><Textarea v-model="form.terms" rows="6" maxlength="1000" :disabled="locked" fluid/></label><label><span>Condiciones particulares del cliente</span><Textarea v-model="form.customerTerms" rows="6" maxlength="10000" :disabled="locked" fluid/></label></div>
    </div>
...
function open(){if(locked.value){visible.value=true;return}profileVersion++;selectedCustomer.value=null;defaults.value=null;profileLoading.value=false;error.value='';Object.assign(form,{entityId:null,salesTarifaId:null,salesTermId:null,date:new Date(),reference:'',reason:'',notes:'',terms:'',customerTerms:'',lines:[newLine()]});visible.value=true;void loadCatalog();}
...
async function selectCustomer(customer:any){
  selectedCustomer.value=customer;form.entityId=customer.pkid;defaults.value=null;form.salesTarifaId=null;form.salesTermId=null;profileLoading.value=true;error.value='';const version=++profileVersion;
