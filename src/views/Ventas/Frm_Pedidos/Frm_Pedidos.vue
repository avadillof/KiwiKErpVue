...
const getCreatorPhotoUrl = (userId: number) => backendUrl(`/gestdoc/users/${userId}/photoPerfil.jpg`);
...
const confirmOrder = () => {
    ...
    confirm.require({ header: 'Confirmar pedido de venta', message: 'El pedido pasará a “Confirmado / En curso”. Podrá modificarse de forma controlada y seguirá por separado su entrega y facturación.', icon: 'pi pi-check-circle', acceptLabel: 'Confirmar pedido', rejectLabel: 'Volver', accept: async () => {
        try { await axios.post(backendUrl(`/WebConfirmSalesOrder/${selectedOrder.value.pkid}`)); toast.add({ severity: 'success', summary: 'Pedido confirmado', detail: 'Ya está preparado para el siguiente paso del circuito.', life: 4000 }); selectedOrder.value = null; tableRef.value?.refresh(); loadOrderStatistics(); }
...
const generateProforma = async () => {
    ...
    try { const { data } = await axios.post(backendUrl(`/WebGenerateSalesProforma/${selectedOrder.value.pkid}`)); selectedOrder.value.proformaCount = Number(selectedOrder.value.proformaCount ?? 0) + 1; tableRef.value?.refresh(); toast.add({ severity: 'success', summary: 'Proforma generada', detail: `Se ha creado la proforma ${data.code}.`, life: 4000 }); const reportUrl = backendUrl(`/WebGetSalesProformaPdf/${data.pkid}`); if (reportWindow) reportWindow.location.href = reportUrl; else window.open(reportUrl, '_blank', 'noopener'); }
...
const openProformas = async () => { if (!selectedOrder.value?.pkid) return; showProformas.value = true; loadingProformas.value = true; try { const { data } = await axios.get(backendUrl(`/WebGetSalesOrderProformas/${selectedOrder.value.pkid}`)); proformas.value = data ?? []; } catch { toast.add({ severity: 'error', summary: 'No se pudieron cargar las proformas', life: 3000 }); } finally { loadingProformas.value = false; } };
const downloadProforma = (proforma: any) => window.open(backendUrl(`/WebGetSalesProformaPdf/${proforma.pkid}`), '_blank', 'noopener');
