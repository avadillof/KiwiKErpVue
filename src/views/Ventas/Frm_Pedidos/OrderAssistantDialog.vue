  ...
  >
    ...
    <template v-if="authorized">
      <Message :severity="aiAvailable ? 'info' : 'warn'" :closable="false">{{
        aiAvailable
          ? "La pregunta se envía a OpenAI para interpretar los filtros. No se envían los resultados ni el historial de pedidos. No incluyas información sensible innecesaria."
      ...
      <section class="assistant-mode-panel">
        <button
          ...
          aria-controls="order-assistant-ai-panel"
          @click="activeMode = 'AI'"
        >
        ...
        >
          ...
          >
          <label for="order-assistant-question"
      ...
      <section class="assistant-mode-panel guided-panel">
        <button
          ...
          aria-controls="order-assistant-guided-panel"
        ...
        <div
          ...
          id="order-assistant-guided-panel"
        ...
        >
          ...
          <form class="criteria" @submit.prevent="consult">
            ...
            <template v-if="criteria.action !== 'EXPLAIN'">
              ...
              <div>
                <label for="assistant-to">Fecha de pedido hasta</label
                ><DatePicker
            ...
            <div v-if="criteria.action === 'EXPLAIN'">
              ...
              ><InputText
                id="assistant-order"
                v-model="criteria.orderCode"
                maxlength="100"
                :disabled="busy"
                :placeholder="
                  criteria.orderId
                    ? `Seleccionada: ${selectedOrder?.code || criteria.orderId}`
                    : 'Introduce el código'
                "
                @update:modelValue="criteria.orderId = null"
              />
            </div>
            <div class="criteria-actions">
              <Button
                v-if="selectedOrder?.pkid"
                label="Usar pedido seleccionado"
                text
                severity="secondary"
                :disabled="busy"
                @click="useSelected"
