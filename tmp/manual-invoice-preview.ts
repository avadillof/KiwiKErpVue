...
axios.defaults.adapter=async config=>{
  ...
  if(url.includes('WebGetSalesQuoteCustomers'))data={content:[{pkid:1,code:'TEST-CLIENT',name:'Cliente de prueba',salesTarifaId:1}],totalElements:1};
  else if(url.includes('WebGetManualSalesInvoiceCustomer'))data={entityId:1,salesTarifaId:1,terms:'Condiciones generales de prueba',customerTerms:'Condiciones particulares de prueba',paymentTermDescription:'30 días',retentionRate:15,fixedTax:null,currencyCode:'EUR'};
  else if(url.includes('WebGetSalesQuoteProducts'))data={content:[{pkid:2,code:'TEST-SERVICE',description:'Servicio de prueba',salePrice:100,taxValue:21}],totalElements:1};
